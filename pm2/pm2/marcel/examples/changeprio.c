...
{ int i, j;
   ...
   tprintf("Inverting Priorities...\n");
   marcel_setprio(marcel_self(), 9);
   for(i=0;i<10;i++) {
      tfprintf(stderr, "Hi %s!\n", arg);
      j = 4000000; while(j--);
   }
...
{ any_t status;
  ...
  marcel_attr_t writer_attr;
   ...
   marcel_create(&writer1_pid, &writer_attr, writer1, (any_t)mess[0]);

   marcel_attr_setprio(&writer_attr, 3);

   marcel_create(&writer2_pid, &writer_attr, writer2,  (any_t)mess[1]);
