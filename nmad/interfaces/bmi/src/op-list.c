...
void op_list_dump(op_list_p olp)
{
    ...
    list_for_each(tmp_entry, olp)
    {
	gossip_print_op(list_entry(tmp_entry, struct method_op,
				   op_list_entry));
    }
}
...
{
    struct list_head *tmp_op_list = NULL;

    tmp_op_list = (struct list_head *) malloc(sizeof(struct list_head));
    if (tmp_op_list)
    {
	INIT_LIST_HEAD(tmp_op_list);
    }
		 ...
		 method_op_p oip)
{
     ...
     */
    list_add_tail(&(oip->op_list_entry), olp);
}
...
void op_list_cleanup(op_list_p olp)
{
    ...
    list_for_each_safe(iterator, scratch, olp)
    {
	tmp_method_op = list_entry(iterator, struct method_op,
				   op_list_entry);
			   ...
			   struct op_list_search_key *key)
{
    ...
    list_for_each(tmp_entry, olp)
    {
	if (!(op_list_cmp_key(key, list_entry(tmp_entry, struct method_op,
					      op_list_entry))))
	{
	    return (list_entry(tmp_entry, struct method_op, op_list_entry));
	}
    }
    return (NULL);
}
...
method_op_p op_list_shownext(op_list_p olp)
{
    ...
    return (list_entry(olp->next, struct method_op, op_list_entry));
}
