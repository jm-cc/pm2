...
static void __fut_dumptime( time_t *the_time, clock_t *the_jiffies)
{
	struct tms cur_time;
...
*/
TBX_VISIBILITY("default") int fut_setup( unsigned int nlongs, unsigned int keymask, unsigned int threadid )
/* TODO: move to libfxt */
{
	unsigned int	nbytes;
	unsigned long	*iptr;
	...
	__fut_dumptime(&fut_infos->start_time, &fut_infos->start_jiffies);
...
*/
TBX_VISIBILITY("default") int fut_keychange( int how, unsigned int keymask, unsigned int threadid )
{
...
*/
TBX_VISIBILITY("default") int fut_done( void )
{
	unsigned int	old_active = fut_active;
...
*/
TBX_VISIBILITY("default") int fut_reset( unsigned int keymask, unsigned int threadid )
{
	unsigned int	nlongs;
...
*/
TBX_VISIBILITY("default") int fut_getbuffer( int *nlongs, unsigned long **buffer )
{
	int				local_nlongs = 0;
...
TBX_VISIBILITY("default") int fut_endup( char *filename )
{
	int n, nlongs=0, fd;
	long size;
	...
	__fut_dumptime(&fut_infos->stop_time,&fut_infos->stop_jiffies);
