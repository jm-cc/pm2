...
int main(int argc, char**argv){
  ...
  if(!rank){ //target
    ...
    for(i = 0; i < nb_tests; ++i){
      memset(buff, 0, BUFF_SIZE * sizeof(int));
      MPI_Put(buff, BUFF_SIZE, MPI_INT, 0, 0, BUFF_SIZE, MPI_INT, win);
      {
	MPI_Win_fence(0, win);
    ...
    { // Single int
      MPI_Win_fence(0, win);
      MPI_Put(buff, 1, MPI_INT, 0, 0, 1, MPI_INT, win);
      MPI_Win_fence(0, win);
    }
    buff[0] = 0;
    { // Array of ints to array of int
      MPI_Win_fence(0, win);
      MPI_Put(buff, 1, mpi_arr_t, 0, 0, BUFF_SIZE, MPI_INT, win);
      MPI_Win_fence(0, win);
    }
    { // Matrix diag to array of int
      MPI_Win_fence(0, win);
      MPI_Put(buff, 1, mpi_diag_t, 0, 0, BUFF_SIDE, MPI_INT, win);
      MPI_Win_fence(0, win);
    }
    { // Matrix diag 2x2 blocks to array of int
      MPI_Win_fence(0, win);
      MPI_Put(buff, BUFF_SIDE / 2, mpi_block_2_t, 0, 0, 2 * BUFF_SIDE, MPI_INT, win);
      MPI_Win_fence(0, win);
    }
    { // Matrix diag 2x2 blocks to array of int
      MPI_Win_fence(0, win);
      MPI_Put(buff, 1, mpi_diag_block_2_t, 0, 0, 2 * BUFF_SIDE, MPI_INT, win);
      MPI_Win_fence(0, win);
    }
    { // Array of int to matrix diag
      MPI_Win_fence(0, win);
      MPI_Put(buff, BUFF_SIDE, MPI_INT, 0, 0, 1, mpi_diag_t, win);
      MPI_Win_fence(0, win);
    }
    { // Array of int to matrix diag 2x2 blocks
      MPI_Win_fence(0, win);
      MPI_Put(buff, 2 * BUFF_SIDE, MPI_INT, 0, 0, BUFF_SIDE / 2, mpi_block_2_t, win);
      MPI_Win_fence(0, win);
    }
    { // Array of int to matrix diag 2x2 blocks
      MPI_Win_fence(0, win);
      MPI_Put(buff, 2 * BUFF_SIDE, MPI_INT, 0, 0, 1, mpi_diag_block_2_t, win);
      MPI_Win_fence(0, win);
    }
