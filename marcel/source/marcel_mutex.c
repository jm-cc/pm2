...
static int pmarcel_mutex_blockcell(pmarcel_mutex_t * mutex,
				   const struct timespec *abstime)
{
	...
	if (timercmp(&tv, &now, <=)) {
		MARCEL_LOG("pmarcel_mutex_blockcell : valeur temporelle invalide\n");
		MARCEL_LOG_RETURN(ETIMEDOUT);
	}

	timeout = (((tv.tv_sec * 1e6 + tv.tv_usec) -
		    (now.tv_sec * 1e6 + now.tv_usec)) + marcel_gettimeslice() -
		   1) / marcel_gettimeslice();

	ma_fastlock_acquire(&mutex->__data.__lock);

	__pmarcel_register_spinlocked(&mutex->__data.__lock, ma_self(), &c);
	...
	if (c.blocked) {
		if (__pmarcel_unregister_spinlocked(&mutex->__data.__lock, &c)) {
			PM2_LOG("Strange, we should be in the queue !!! (%s:%d)\n",
				__FILE__, __LINE__);
		}
		//on sort       
		ma_fastlock_release(&mutex->__data.__lock);
...
{
	MARCEL_LOG_IN();
	MARCEL_LOG_RETURN(pmarcel_mutex_unlock_usercnt(mutex, 1));
}
