 ...
 */
int marcel_init_timer(struct marcel_timer_list *timer, void (*function)(unsigned long id), 
...
static inline void __run_timers(ma_tvec_base_t * base)
{
	...
	while (ma_time_after_eq(ma_jiffies, base->timer_jiffies)) {
		...
		while (!tbx_fast_list_empty(head)) {
			...
			timer = tbx_fast_list_entry(head->next, struct marcel_timer_list, entry);
			fn = timer->function;
			data = timer->data;
...
fastcall signed long ma_schedule_timeout(signed long timeout)
{
	...
	switch (timeout) {
	case MARCEL_MAX_SCHEDULE_TIMEOUT:
		/*
