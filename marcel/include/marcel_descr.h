...
#define TIF_DEBUG_IN_PROGRESS   16	/* true if PM2_LOG is in execution */
...
struct ma_task {
	...
	struct marcel_timer_list schedule_timeout_timer;

	/** \brief Base of the thread stack. */
	any_t stack_base;

	/** \brief Size of stack pointed by stack_base */
	size_t stack_size;
...
#ifdef MA__LIBPTHREAD
	struct _marcel_cleanup_buffer *lpt_last_cleanup;
#endif
	...
	char tls[MARCEL_TLS_AREA_SIZE] TBX_ALIGN(16);
#else
#if defined(X86_64_ARCH) && defined(MA__LWPS) && defined(__GLIBC__)
	/* Space to store SSE registers: accessed by 
	 * _dl_x86_64_{save/restore}_sse() functions
	 * nptl/sysdeps/x86_64/tls.h */
	__m128 ma_rtld_savespace_sse[8][4];
