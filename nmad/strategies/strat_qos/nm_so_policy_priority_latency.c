...
{
  struct nm_so_policy_priority_latency_gate *priv = 
    TBX_MALLOC(sizeof(struct nm_so_policy_priority_latency_gate));
  uint8_t i;
...
{
  struct nm_so_policy_priority_latency_gate *priv = addr_private[0];
...
busy(void *private)
{
  return ((struct nm_so_policy_priority_latency_gate *)private)->nb_packets;
}

nm_so_policy nm_so_policy_priority_latency = {
  .pack_ctrl = pack_ctrl,
  .pack = pack,
