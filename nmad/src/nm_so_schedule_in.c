...
static inline void nm_core_unpack_check_completion(struct nm_core*p_core, struct nm_pkt_wrap_s*p_pw, struct nm_req_s**pp_unpack, nm_len_t chunk_len)
{
...
/** unpack an unexpected chunk to a matches req */
static void nm_core_unpack_unexpected(struct nm_core*p_core, struct nm_req_s**pp_unpack, struct nm_unexpected_s*p_unexpected)
{
  struct nm_req_s*p_unpack = *pp_unpack;
...
/** checks whether an unpack request is ready to receive data, store chunk as unexpected otherwise */
static inline int nm_core_unpack_ready_to_receive(struct nm_core*p_core, struct nm_req_s*p_unpack,
						  nm_gate_t p_gate, const struct nm_header_generic_s*p_header,
						  nm_len_t chunk_offset, nm_len_t chunk_len, nm_core_tag_t tag, nm_seq_t seq,
						  struct nm_pkt_wrap_s*p_pw)
{
  if((p_unpack != NULL) &&
  ...
  nm_unexpected_store(p_core, p_gate, p_header, chunk_offset, chunk_len, tag, seq, p_pw);
  if((p_unpack != NULL) && (p_unpack->unpack.expected_len == NM_LEN_UNDEFINED))
    {
      /* data spec still undefined- fire data event */
      const struct nm_core_event_s event =
	{
...
void nm_pw_process_complete_recv(struct nm_core*p_core, struct nm_pkt_wrap_s*p_pw)
{
  ...
  if(p_pw->trk_id == NM_TRK_SMALL)
  ...
  else if(p_pw->trk_id == NM_TRK_LARGE)
    {
      ...
      nm_core_unpack_check_completion(p_core, p_pw, &p_unpack, len);
    }
  nm_pw_ref_dec(p_pw);
