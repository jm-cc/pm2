...
#ifndef TYPE
# define TYPE PMARCEL_MUTEX_NORMAL
#endif
...
static int do_test(void)
{
	...
	if (pmarcel_mutexattr_gettype(&a, &mtype) != 0) {
		puts("mutexattr_gettype failed");
		return 1;
	}
	if (mtype != PMARCEL_MUTEX_DEFAULT) {
		puts("default type must be PMARCEL_MUTEX_DEFAULT");
		return 1;
	}
	...
	err = pmarcel_mutex_timedlock(&m, &ts);
	if (err == 0) {
	...
	ts.tv_sec += 2;		/* Wait 2 seconds.  */
	if (pmarcel_mutex_timedlock(&m, &ts) != 0) {
