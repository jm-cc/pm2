...
[
	...
	if test $__with_$1 = $5; then
	   	echo $5 | grep - >/dev/null 2>&1
	   	if test $? -ne 0; then
		   __opt_$5=yes
		fi
	   	AC_MSG_RESULT(set $4 to... $__with_$1)
	else
	   	echo $withval | grep - >/dev/null 2>&1
	   	if test $? -ne 0; then
...
[
	if test x$__with_$1 != xno; then
		ADD_TO(PKG_AM_CPPFLAGS, -D$2=$__with_$1)
	fi
])


# OPTION_SET_POSITIVE_CONST(ARGNAME, DEFINENAME)
# ----------------------------------------------
# Add a CPPFLAGS -DDEFINENAME=ARGNAME_value if ARGNAME_value is strictly positive
