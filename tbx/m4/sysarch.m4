...
AC_DEFUN([TARGET_SYS_ARCH], [dnl
  ...
  ADD_TO(PKG_AM_CPPFLAGS, -D$TARGET_ARCH -D$TARGET_SYS)
  ...
  if test $TARGET_SYS = LINUX_SYS; then
     ...
     ADD_TO(GLOBAL_AM_CPPFLAGS, -DLINUX_VERSION=\"\\\"$VERSION.$PATCHLEVEL\\\"\" -D_GNU_SOURCE=1)
     if test "$VERSION" -lt 2 -o "$PATCHLEVEL" -lt 6; then
     	ADD_TO(GLOBAL_AM_CPPFLAGS, -DOLD_ITIMER_REAL)
     fi
  fi
