...
#define BUFFER_LENGTH_MIN  4 //4
#define BUFFER_LENGTH_MAX  2048 //512 //(2*1024*1024) //32768
...
client(p_mad_channel_t channel){
    ...
    while(cur_length <=  BUFFER_LENGTH_MAX){
        while(cur_nb_packs <= cur_length
              && cur_nb_packs <= NB_PACKS) {

            TBX_GET_TICK(t1);
            while (counter < NB_LOOPS) {

                connection1 = mad_begin_packing(channel, 0);
                for(i = 0; i < cur_nb_packs; i++){
                    mad_pack(connection1,
                             buffer_e,
                             cur_length / cur_nb_packs,
                             mad_send_CHEAPER,
                             mad_receive_CHEAPER);
                }
            ...
            // on passe à un nombre de packs supérieur
            cur_nb_packs*=2;
            counter = 0;
        }
        cur_length *= 2;
