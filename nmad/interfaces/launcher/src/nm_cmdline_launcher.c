...
void nm_cmdline_launcher_init(void*_status, int *argc, char **argv, const char*_label)
{
  ...
  if(!remote_launcher_url)
      ...
      if(rc) 
	{
      ...
      if(fd < 0)
	{
	  padico_fatal("Topology: cannot open netlink socket (socket: %s).\n", strerror(errno));
	}
	...
	.nl_groups = 0
      ...
      rc = bind(fd, (struct sockaddr*)&nladdr, sizeof(addr));
      if(rc < 0)
	{
	  padico_fatal("Topology: cannot bind netlink socket.\n");
	}
      /* netlink request */
      struct 
      {
	...
	.msg_namelen = sizeof(peer)
      ...
      rc = sendmsg(fd, &msg, 0);
      if(rc < 0)
	padico_fatal("Topology: error while sending netlink request.\n");
      /* receive reply */
      char buf[8192];
