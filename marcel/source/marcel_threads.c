...
#if defined(MA__PROVIDE_TLS) && defined(LINUX_SYS)
...
DEF_PMARCEL(pmarcel_t, self, (void), (), 
{
	return (pmarcel_t)ma_self();
})
...
/** thread identity **/
int marcel_thread_getid(marcel_t thread)
{
	return thread->id;
}

void marcel_thread_setid(marcel_t thread, int id)
{
	thread->id = id;
}

/** thread number **/
int marcel_thread_number(marcel_t thread)
{
...
{
	struct ma_entity initentity = MA_SCHED_ENTITY_INITIALIZER(initentity, MA_THREAD_ENTITY, MA_DEF_PRIO, "");

	MARCEL_LOG_IN();
	PROF_THREAD_BIRTH(t);

	/* Define thread entity schedule parameters */
	t->as_entity = initentity;
	...
	t->detached = (attr->__flags & MA_ATTR_FLAG_DETACHSTATE) ? MARCEL_CREATE_DETACHED : MARCEL_CREATE_JOINABLE;
	if (t->detached != MARCEL_CREATE_DETACHED)
		marcel_sem_init(&t->client, 0);
	else
		marcel_sem_init(&t->client, -1);
	//t->ret_val
...
#  endif
	...
	//t->tls
#if !defined(MA__PROVIDE_TLS) && defined(X86_64_ARCH) && defined(MA__LWPS) && defined(__GLIBC__)
	t->rtld_must_xmm_save = MA_GET_TCB(rtld_must_xmm_save);
#endif
		       ...
		       __const unsigned long base_stack)
{
	...
	if (attr->seed) {
		struct ma_entity initentity = MA_SCHED_ENTITY_INITIALIZER(initentity,
		...
		/* seed can be joined */
#ifdef MARCEL_POSTEXIT_ENABLED
		new_task->postexit_func = NULL;
#endif

		/* Intialize thread seed structure */
		new_task->as_entity = initentity;
			
		new_task->cur_thread_seed = NULL;
		new_task->f_to_call = func;
		new_task->arg = arg;
		new_task->stack_kind = MA_NO_STACK;
		if (attr->__flags & MA_ATTR_FLAG_DETACHSTATE) {
			new_task->detached = MARCEL_CREATE_DETACHED;
			marcel_sem_init(&new_task->client, -1);
		}
		else {
			new_task->detached = MARCEL_CREATE_JOINABLE;
...
DEF_MARCEL(int,create,(marcel_t * __restrict pid, __const marcel_attr_t * __restrict attr,
		       marcel_func_t func, any_t __restrict arg),(pid,attr,func,arg),
{
	MARCEL_LOG_IN();
	MARCEL_LOG_RETURN(marcel_create_internal(pid, attr, func, arg, 0, (unsigned long)&arg));
})

DEF_PMARCEL(int,create,(pmarcel_t * __restrict pid, __const pmarcel_attr_t * __restrict attr,
			marcel_func_t func, any_t __restrict arg),(pid,attr,func,arg),
{
	int status;
	...
	if (attr) {
		...
		if (new_attr.__flags & MA_ATTR_FLAG_INHERITSCHED) {
			...
			pmarcel_getschedparam((pmarcel_t)ma_self(), &new_attr.__schedpolicy, &new_attr.__schedparam);
			pmarcel_attr_setschedparam(&new_attr, &new_attr.__schedparam);

			/* and scope
			 * NB: spare vp scope is SYSTEM */
			if ((ma_vpnum(MA_LWP_SELF) >= 0) && (ma_vpnum(MA_LWP_SELF) < (int)marcel_nb_vp))
				pmarcel_attr_setscope(&new_attr, PMARCEL_SCOPE_PROCESS);
			else
				pmarcel_attr_setscope(&new_attr, PMARCEL_SCOPE_SYSTEM);
		}

		/* disable preemption if necessary */
		if (new_attr.__schedpolicy == SCHED_FIFO)
			marcel_attr_setpreemptible(&new_attr, 0);

#ifdef MA__LWPS
		if (new_attr.__flags & MA_ATTR_FLAG_SCOPESYSTEM) {
			marcel_vpset_t full = MARCEL_VPSET_FULL;
			unsigned lwp = marcel_lwp_add_vp(full);
...
int marcel_create_dontsched(marcel_t * __restrict pid, __const marcel_attr_t * __restrict attr,
			    marcel_func_t func, any_t __restrict arg)
{
	return marcel_create_internal(pid, attr, func, arg, 1, (unsigned long)&arg);
}

int marcel_create_special(marcel_t * __restrict pid, __const marcel_attr_t * __restrict attr,
			  marcel_func_t func, any_t __restrict arg)
{
	return marcel_create_internal(pid, attr, func, arg, 2, (unsigned long)&arg);
}
...
static void* postexit_thread_func(any_t arg)
{
	...
	while(1) {
		...
		if (!ma_topo_vpdata_l(vp,postexit_func)) {
			...
			(*ma_topo_vpdata_l(vp,postexit_func))(ma_topo_vpdata_l(vp,postexit_arg));
...
static void marcel_threads_postexit_start(marcel_lwp_t *lwp)
{
	...
	marcel_snprintf(name,MARCEL_MAXNAMESIZE,"postexit/%d",ma_vpnum(lwp));
...
#  endif
	...
	if (task->cur_thread_seed) {
		task->cur_thread_seed->ret_val = val;

		/* make sure waiter doesn't start before we're off. */
		ma_preempt_disable();

		int detached = task->cur_thread_seed->detached;
#ifdef MA__DEBUG
		/* NOTE: this is quite costly, only do it to get gdb's
		 * marcel-threads & marcel_top working */
		marcel_one_task_less(task->cur_thread_seed);
#endif

		marcel_funerals(task->cur_thread_seed);
		if (detached != MARCEL_CREATE_DETACHED)
			marcel_sem_V(&task->cur_thread_seed->client);
	} else {
...
#ifdef MARCEL_CLEANUP_ENABLED
		while (task->last_cleanup) {
			_marcel_cleanup_pop(task->last_cleanup, 1);
		}

#ifdef MA__LIBPTHREAD
		while (task->lpt_last_cleanup) {
			ma_lpt_cleanup_pop(task->lpt_last_cleanup, 1);
		}
#endif
#endif /* MARCEL_CLEANUP_ENABLED */

#ifdef MARCEL_ATEXIT_ENABLED
		marcel_atexit_exec(task);
#endif /* MARCEL_ATEXIT_ENABLED */
	...
	/* key handling */
		{
			int key;
			int key_processed;
			int key_destr_iter = 0;
			
			do {
				key_processed = 0;
				for (key = 0; key < MARCEL_MAX_KEYS; key++) {
					if (marcel_key_destructor[key] && task->key[key]) {
						(*(marcel_key_destructor[key]))(task->key[key]);
						key_processed ++;
					}
				}
				key_destr_iter++;
			} while (key_processed && (key_destr_iter < MAX_KEY_DESTR_ITER));
		}
...
#ifdef MARCEL_POSTEXIT_ENABLED
		if (task->postexit_func) {
			ma_preempt_disable();
			vp = ma_get_task_lwp(task)->vp_level;
...
#  endif
			...
			ma_topo_vpdata_l(vp,postexit_func)=task->postexit_func;
			ma_topo_vpdata_l(vp,postexit_arg)=task->postexit_arg;
		}
#endif /* MARCEL_POSTEXIT_ENABLED */
		 ...
		 */
		if (task->detached != MARCEL_CREATE_DETACHED)
			marcel_sem_V(&task->client);
#else
		/** unlock thread which are joining main thread **/ 
		if (task == __main_thread)
			marcel_sem_V(&task->client);
#endif
		ma_preempt_enable();
		if (tbx_unlikely(task == __main_thread || ma_fork_generation)) {
	...
	/* go off */
	ma_set_task_state(task, MA_TASK_DEAD);
	if (task == ma_self())
		ma_schedule();
}

DEF_MARCEL(void TBX_NORETURN, exit, (any_t val), (val),
{
        ma_exit_internal(ma_self(), val);
	abort(); // For security
})

DEF_PMARCEL(void TBX_NORETURN, exit, (any_t val), (val),
{
        ma_exit_internal(ma_self(), val);
	abort(); // For security
})
...
#ifdef MA__LWPS
	/* see ma_exit_internal for the mono case */
	else
...
static int check_join(marcel_t tid)
{
	...
	/** thread with attribute CREATE_DETACHED => semval == -1 */
	marcel_sem_getvalue(&tid->client, &joinsem);
	if (joinsem == -1) {
		MARCEL_LOG("check_join: thread %p is already detached\n", tid);
		return EINVAL;
	}

	/** check that joinable thread was not previously detached/joined */
	if (MARCEL_THREAD_ISALIVE(tid)) {
		if (tid->detached == MARCEL_CREATE_DETACHED)
			return EINVAL;
		else
			return 0;
	}

	MARCEL_LOG("check_join: thread %p is dead\n", tid);
	return ESRCH;
}
...
DEF_MARCEL(int, join, (marcel_t pid, any_t *status), (pid, status),
{
	...
	MA_BUG_ON(pid->detached != MARCEL_CREATE_JOINABLE);
...
static int check_detach(marcel_t tid)
{
	...
	marcel_sem_getvalue(&tid->client, &joinsem);
	if (joinsem == -1) {
		MARCEL_LOG("check_detach: thread %p is already detached\n", tid);
		return EINVAL;
	}
	...
	MARCEL_LOG("check_detach: thread %p is dead\n", tid);
	return ESRCH;
}
...
DEF_MARCEL_PMARCEL(void, cleanup_pop,(struct _marcel_cleanup_buffer *__buffer, int execute), 
		   (__buffer, execute),
{
	MARCEL_LOG_IN();
	marcel_t cur = ma_self();
...
#    endif
DEF_PTHREAD(void, cleanup_pop,(struct _pthread_cleanup_buffer *__buffer, int __execute), 
	    (__buffer, __execute))
...
static void main_thread_init(void)
{
	...
	if (__main_thread == NULL) {
		MA_WARN_USER("Couldn't find main thread's marcel_t, i.e. Marcel's main was not called yet. Please either load the stackalign module early, or use the STANDARD_MAIN option (but this will decrease performance)\n");
		abort();
	}
#endif
	...
	*((marcel_t *)(ma_task_slot_top(__main_thread) - sizeof(void *))) = __main_thread;
	...
	{
		/* region returned to the user is [get_sp() - rlimit_stacksize; get_sp() + rlimit_stacksize]
		 * maybe some variables have been pushed on the thread stack */
		struct rlimit rlp;
		getrlimit(RLIMIT_STACK, &rlp);		
		__main_thread->stack_size = 2*rlp.rlim_cur;
	}
#else
...
#endif /* MARCEL_DEVIATION_ENABLED && MARCEL_POSIX */
...
/*************************setschedprio************************/
DEF_MARCEL(int, setschedprio,(marcel_t thread,int prio),(thread,prio),
{
	MARCEL_LOG_IN();
	int policy;
...
DEF_PMARCEL(int, setschedprio,(pmarcel_t thread,int prio),(thread,prio),
{
	return marcel_setschedprio((marcel_t)thread, prio);
})
...
{
	strncpy((pid->as_entity).name, name, MARCEL_MAXNAMESIZE - 1);
	(pid->as_entity).name[MARCEL_MAXNAMESIZE - 1] = '\0';
...
{
	strncpy(name, (pid->as_entity).name, n);
