...
DEF_PMARCEL(pmarcel_t, self, (void), (), {
	    MARCEL_LOG_IN();
	    MARCEL_LOG_RETURN((pmarcel_t) (marcel_self()));
})
#ifdef MA__LIBPTHREAD
#include <sys/syscall.h>
extern int __pthread_create_2_1(pthread_t *, const pthread_attr_t *, void *(*)(void *),
				void *);
			 ...
			 void *(*start_routine) (void *), void *arg)
{
	...
	if (attr != NULL) {
		...
		memcpy(&new_attr, attr,
		       tbx_offset_of(marcel_attr_t,
				     __stacksize) + sizeof(new_attr.__stacksize));
		...
		if (new_attr.__flags & MA_ATTR_FLAG_INHERITSCHED) {
			...
			if (marcel_vpset_isfull(&vpset))
				marcel_attr_setscope(&new_attr, PTHREAD_SCOPE_PROCESS);
			else
				marcel_attr_setscope(&new_attr, PTHREAD_SCOPE_SYSTEM);
			...
			if (mprio >= MA_DEF_PRIO)
				...
				if (ma_some_thread_is_preemption_disabled(cthread))
					policy = SCHED_FIFO;
				else
					policy = SCHED_RR;
			}
...
lpt_t LPT_NAME(self) (void) {
	MARCEL_LOG_IN();
	MARCEL_LOG_RETURN((lpt_t) pmarcel_self());
}
...
#define HAVE_PTR_NTHREADS
	...
	.ptr___pthread_cleanup_upto = (void (*)(long *, char *))error_stub,
...
{
	MARCEL_LOG_IN();
	MARCEL_LOG_RETURN(thread1 == thread2);
}
...
off64_t __lseek64(int fd, off64_t offset, int whence)
{
	...
	if (tbx_unlikely(!marcel_createdthreads())) {
		ret =
		    syscall(SYS__llseek, fd, (off_t) (offset >> 32),
			    (off_t) (offset & 0xffffffff), &res, whence);
	} else {
		int old = __pmarcel_enable_asynccancel();
		ret =
		    syscall(SYS__llseek, fd, (off_t) (offset >> 32),
...
DEF_STRONG_ALIAS(__lseek64, lseek64)
...
DEF_STRONG_ALIAS(__llseek, llseek)
#else
DEF_WEAK_ALIAS(LPT_NAME(lseek), lseek64)
DEF_WEAK_ALIAS(LPT_NAME(lseek), llseek)
#endif
...
cancellable_call_ext(int, open64, SYS_open, GLIBC_2_2,
		     (const char *path, int flags, mode_t mode), path, flags, mode)
#ifdef SYS_pread
cancellable_call_ext(ssize_t, pread, SYS_pread, GLIBC_2_2,
		     (int fd, void *buf, size_t count, off_t pos), fd, buf, count, pos)
#endif
#ifdef SYS_pread64
cancellable_call_ext(ssize_t, pread64, SYS_pread64, GLIBC_2_2,
		     (int fd, void *buf, size_t count, off64_t pos), fd, buf, count, pos)
#endif
#ifdef SYS_pwrite
cancellable_call_ext(ssize_t, pwrite, SYS_pwrite, GLIBC_2_2,
		     (int fd, const void *buf, size_t count, off_t pos), fd, buf, count,
		     pos)
#endif
#ifdef SYS_pwrite64
cancellable_call_ext(ssize_t, pwrite64, SYS_pwrite64, GLIBC_2_2,
		     (int fd, const void *buf, size_t count, off64_t pos), fd, buf, count,
		     pos)
#endif
cancellable_call(ssize_t, read, (int fd, void *buf, size_t count), fd, buf, count)
#ifdef SYS_waitpid
cancellable_call(pid_t, waitpid, (pid_t pid, int *status, int options), pid, status,
		 options)
#elif SYS_wait4
...
cancellable_call_generic(pid_t, waitpid,
			 ...
			 GLIBC_2_2_5,
			 (pid_t pid, int *status, int options), pid, status, options)
#endif
...
#endif
#ifdef SYS_sendto
cancellable_call(ssize_t, sendto,
		 (int s, const void *buf, size_t len, int flags,
		  const struct sockaddr * to, socklen_t tolen), s, buf, len, flags, to,
		 tolen)
#endif
#ifdef SYS_recvfrom
cancellable_call(ssize_t, recvfrom,
		 (int s, void *buf, size_t len, int flags, struct sockaddr * from,
		  socklen_t * fromlen), s, buf, len, flags, from, fromlen)
#endif
...
__pthread_register_cancel(__pthread_unwind_buf_t * buf TBX_UNUSED)
{
	MA_BUG();
}
