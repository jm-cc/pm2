		...
		p_mad_ping_result_t results)
{
  ...
  if (process_lrank == master_lrank)
      ...
      if (!results)
	{
	  ...
	  ntbx_pack_buffer_t b1;
	  ntbx_pack_buffer_t b2;
	  ntbx_pack_buffer_t b3;
	  ntbx_pack_buffer_t b4;
	  ntbx_pack_buffer_t b5;
	  ntbx_pack_buffer_t b6;
	  ntbx_pack_buffer_t b7;
	  ntbx_pack_buffer_t b8;

	  in = mad_begin_unpacking(channel);
#define _UNPACK(b) mad_unpack(in, &(b), sizeof((b)), mad_send_CHEAPER, mad_receive_CHEAPER)
	  _UNPACK(b1);
	  _UNPACK(b2);
	  _UNPACK(b3);
	  _UNPACK(b4);
...
#undef _UNPACK
	  ...
	  tmp_results.lrank_src       = ntbx_unpack_int   (&b1);
	  tmp_results.lrank_dst       = ntbx_unpack_int   (&b2);
	  tmp_results.size            = ntbx_unpack_int   (&b3);
	  tmp_results.nb_pack         = ntbx_unpack_int   (&b4);
	  tmp_results.nb_msg          = ntbx_unpack_int   (&b5);
	  tmp_results.latency         = ntbx_unpack_double(&b6);
	  tmp_results.bandwidth_mbit  = ntbx_unpack_double(&b7);
	  tmp_results.bandwidth_mbyte = ntbx_unpack_double(&b8);
  ...
  else if (results)
    {
      ...
      ntbx_pack_int(results->lrank_src,          &b1);
      ntbx_pack_int(results->lrank_dst,          &b2);
      ntbx_pack_int(results->size,               &b3);
      ntbx_pack_int(results->nb_pack,            &b4);
      ntbx_pack_int(results->nb_msg,             &b5);
      ntbx_pack_double(results->latency,         &b6);
      ntbx_pack_double(results->bandwidth_mbit,  &b7);
      ntbx_pack_double(results->bandwidth_mbyte, &b8);

      out = mad_begin_packing(channel, master_lrank);
#define _PACK(b) mad_pack(out, &(b), sizeof((b)), mad_send_CHEAPER, mad_receive_CHEAPER)
      _PACK(b1);
      _PACK(b2);
      _PACK(b3);
      _PACK(b4);
...
#undef _PACK
      ...
      while (ntbx_pc_next_local_rank(pc, &lrank_src))
	{
	  ...
	  out = mad_begin_packing(channel, lrank_src);
	  mad_pack(out, &b1, sizeof(b1),
		   mad_send_CHEAPER, mad_receive_CHEAPER);
	  mad_pack(out, &b2, sizeof(b2),
		   mad_send_CHEAPER, mad_receive_CHEAPER);
	  mad_end_packing(out);
	}
    }
      ...
      while (1)
	{
	  ...
	  in = mad_begin_unpacking(channel);
	  mad_unpack(in, &b1, sizeof(b1),
		     mad_send_CHEAPER, mad_receive_CHEAPER);
	  mad_unpack(in, &b2, sizeof(b2),
		     mad_send_CHEAPER, mad_receive_CHEAPER);
	  mad_end_unpacking(in);

	  its_local_rank = ntbx_unpack_int(&b1);
	  direction = ntbx_unpack_int(&b1);
