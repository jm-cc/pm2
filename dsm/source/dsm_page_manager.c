  ...
  case DSM_CUSTOM :
   ...
   if (_dsm_page_protect_mode != DSM_OWNER_WRITE_ACCESS_OTHER_NO_ACCESS && _dsm_page_protect_mode != DSM_OWNER_READ_ACCESS_OTHER_NO_ACCESS) 
     ...
     case DSM_CUSTOM_PROTECT:
       MARCEL_EXCEPTION_RAISE(MARCEL_NOT_IMPLEMENTED);break;
     }
}
...
void dsm_set_next_owner(dsm_page_index_t index, dsm_node_t next)
{
...
void dsm_store_pending_request(dsm_page_index_t index, dsm_node_t node)
...
void dsm_enable_page_entry(dsm_page_index_t index, dsm_node_t owner, dsm_proto_t protocol, void *addr, size_t size, tbx_bool_t map)
{
 ...
 // fprintf(stderr,"Allocated entry, @=%p\n", dsm_page_table[index]);
  ...
  case DSM_CUSTOM_PROTECT: MARCEL_EXCEPTION_RAISE(MARCEL_NOT_IMPLEMENTED);break;
  }
  /* Perform protocol-specific initializations. */
