...
#ifndef __INLINEFUNCTIONS_SYS_LINUX_INTERRUPT_H__
#define __INLINEFUNCTIONS_SYS_LINUX_INTERRUPT_H__
...
static __tbx_inline__ void ma_remote_tasklet_set_vpset(struct marcel_tasklet_struct *t,
...
#ifdef MA__LWPS
static __tbx_inline__ int ma_tasklet_trylock(struct marcel_tasklet_struct *t)
{
...
/*************************************************************************************
 * static __tbx_inline__ int ma_tasklet_unlock(struct marcel_tasklet_struct *t) {        *
 ...
 * #else                                                                             *
 *************************************************************************************/
static __tbx_inline__ void ma_tasklet_unlock(struct marcel_tasklet_struct *t)
{
	ma_smp_mb__before_clear_bit();
	ma_clear_bit(MA_TASKLET_STATE_RUN, &(t)->state);
}

static __tbx_inline__ void ma_tasklet_unlock_wait(struct marcel_tasklet_struct *t)
{
	while (ma_test_bit(MA_TASKLET_STATE_RUN, &(t)->state)) {
		ma_barrier();
	}
}
#else
...
static __tbx_inline__ void __ma_tasklet_remote_schedule(struct marcel_tasklet_struct *t,
							unsigned vpnum)
{
	ma_lwp_t lwp;
	...
	if (t->preempt && tbx_likely(lwp = ma_get_lwp_by_vpnum(vpnum)))
		MA_LWP_RESCHED(lwp);

	ma_remote_tasklet_unlock(&vp->tasklet_lock);
}

static __tbx_inline__ void ma_tasklet_schedule(struct marcel_tasklet_struct *t)
{
	...
	old_state =
	    ma_xchg(&t->state,
		    (1 << MA_TASKLET_STATE_SCHED) | (1 << MA_TASKLET_STATE_RUN));
	switch (old_state) {
	case 0:{
			unsigned long target_vp_num;
			//DISP("tasklet schedule, t = %p", t);
			/* not running, not scheduled, schedule it here */
			if (marcel_vpset_isset(&(t->vp_set), ma_vpnum(MA_LWP_SELF))) {
				ma_clear_bit(MA_TASKLET_STATE_RUN, &(t)->state);
				ma_remote_tasklet_unlock(&vp->tasklet_lock);
...
static __tbx_inline__ void ma_tasklet_hi_schedule(struct marcel_tasklet_struct *t)
{
	unsigned long old_state;
	old_state =
	    ma_xchg(&t->state,
...
static __tbx_inline__ void ma_tasklet_disable_nosync(struct marcel_tasklet_struct *t)
{
	ma_atomic_inc(&t->count);
	ma_smp_mb__after_atomic_inc();
}

static __tbx_inline__ void ma_tasklet_disable(struct marcel_tasklet_struct *t)
{
...
static __tbx_inline__ void ma_tasklet_enable(struct marcel_tasklet_struct *t)
{
	ma_smp_mb__before_atomic_dec();
	ma_atomic_dec(&t->count);
}

static __tbx_inline__ void ma_tasklet_hi_enable(struct marcel_tasklet_struct *t)
{
	ma_smp_mb__before_atomic_dec();
	ma_atomic_dec(&t->count);
}
...
#endif /** __INLINEFUNCTIONS_SYS_LINUX_INTERRUPT_H__ **/
