...
void pingpong_datatype_struct(struct nm_core        *p_core,
  ...
  if (client) {
    ...
    TBX_GET_TICK(t1);
    for(k = 0 ; k<LOOPS ; k++) {
      pack_datatype_struct(p_core, gate_id, datatype_struct, buffer);
      unpack_datatype_struct(p_core, gate_id, r_buffer);
    }
    TBX_GET_TICK(t2);
...
void pack_datatype_struct(struct nm_core        *p_core,
                          uint8_t                gate_id,
  ...
  DEBUG("Sending struct datatype, count=%d indices[0]=%d blocklen[0]=%d oldtype[0].size=%d elements=%d at address %p\n",
  ...
  for(i=0 ; i<numberOfBlocks ; i++) {
    ...
    DEBUG("Send data for block %d : %d elements from indice %d with size %d\n", i, datatype->blocklens[i], datatype->indices[i], size);
...
void unpack_datatype_struct(struct nm_core  *p_core,
                            uint8_t          gate_id,
                            particle_t      *r_ptr) {
  struct nm_so_cnx  *cnx = NULL;
  ...
  DEBUG("Receiving struct type at address %p...\n", r_ptr);
  ...
  DEBUG("Count %d - Number of blocks %d\n", count, numberOfBlocks);
  ...
  for(i=0 ; i<numberOfBlocks ; i++) {
    DEBUG("Received data for block %d : %d elements from indice %d with size %d\n", i, block_elements[i], block_offset[i], block_size[i]);
  }
  ...
  DEBUG("End of unpacking ....\n");
  nm_so_end_unpacking(p_core, cnx);
  DEBUG("After unpacking ....\n");
