...
int lpt_mutex_destroy(lpt_mutex_t * mutex TBX_UNUSED)
{
	MARCEL_LOG_IN();
	MARCEL_LOG_RETURN(0);
}
...
static int lpt_mutex_blockcell(lpt_mutex_t * mutex, const struct timespec *abstime)
{
...

	{
		...
		__lpt_register_spinlocked(&mutex->__data.__lock, ma_self(), &c);
...
{
	MARCEL_LOG_IN();
	MARCEL_LOG_RETURN(lpt_mutex_unlock_usercnt(mutex, 1));
}
