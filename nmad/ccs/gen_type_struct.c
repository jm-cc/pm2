					   ...
					   int flags)
{
    ...
    DLOOP_Type first_basic = CCS_DATA_NULL,
	first_derived = CCS_DATA_NULL;
    ...
    for (i=0; i < count; i++)
    {
	if (oldtypes[i] != CCS_DATA_LB && oldtypes[i] != CCS_DATA_UB)
	{
	    int is_builtin;
     ...
     * the DLOOP_Type, we can safely ignore them in all our
     * calculations here.
     */

    /* optimization:
     *
     * if there were only LBs and UBs in the struct type,
     ...
     *
     */
    if (((nr_derived == 0) && (first_basic != CCS_DATA_NULL)) ||
	((nr_basics == 0) && (first_derived != CCS_DATA_NULL)))
    {
     ...
     */
    if (nr_derived == 0 && ((flags & CCSI_DATALOOP_HOMOGENEOUS) ||
			    (flags & CCSI_DATALOOP_ALL_BYTES)))
    {
     ...
     */
    if ((flags & CCSI_DATALOOP_HOMOGENEOUS) ||
	     (flags & CCSI_DATALOOP_ALL_BYTES))
    {
    ...
    for (i=0; i < count; i++)
    {
	if (CCS_datadesc_is_builtin (oldtypes[i]))
	{
	    int tmp_loop_depth, tmp_loop_sz;

	    DLOOP_Handle_get_loopdepth_macro(oldtypes[i], tmp_loop_depth, 0);
	    DLOOP_Handle_get_loopsize_macro(oldtypes[i], tmp_loop_sz, 0);
    ...
    new_dlp->el_type = CCS_DATA_NULL; /* not valid for struct */
	...
	else
	{
	    ...
	    DLOOP_Handle_get_loopptr_macro(oldtypes[i], old_loop_ptr, 0);
	    DLOOP_Handle_get_loopsize_macro(oldtypes[i], old_loop_sz, 0);
	...
	new_dlp->loop_params.s_t.offset_array[loop_idx] = disps[i];
	loop_idx++;
    }
	       ...
	       int flags)
{
	...
	if (oldtypes[i] != CCS_DATA_LB && oldtypes[i] != CCS_DATA_UB)
	{
	    ...
	    DLOOP_Handle_get_basic_size_macro(oldtypes[i], sz);
						  ...
						  int flags)
{
	...
	if (is_basic && (oldtypes[i] != CCS_DATA_LB &&
			 oldtypes[i] != CCS_DATA_UB))
	{
	    nr_blks++;
	}
	 ...
	 */
	if (oldtypes[i] != CCS_DATA_UB && oldtypes[i] != CCS_DATA_LB)
	{
