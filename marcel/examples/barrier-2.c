...
worker (void *arg)
{
  ...
  for (i = 0; i < ROUNDS; ++i)
	...
	{
	  fprintf(stderr,"thread %d failed to wait for all the others\n", nr);
	  result = (void *) 1;
	}
      ...
      for (j = nr; j < NTHREADS; ++j)
	{
	  ...
	  if (nr == 0 && counters[j] != j + 1)
	    {
	      fprintf(stderr,"barrier in round %d released but count is %d\n",
		      j, counters[j]);
	      result = (void *) 1;
	    }
	  ...
	  if (retval != 0)
		...
		{
		  fprintf(stderr,"thread %d in round %d has nonzero return value != PTHREAD_BARRIER_SERIAL_THREAD\n",
			  nr, j);
		  result = (void *) 1;
		}
	      else
	  ...
	  if (nr == 0 && serial[j] != 1)
	    {
	      fprintf(stderr,"not exactly one serial thread in round %d\n", j);
	      result = (void *) 1;
	    }
	}
    }
...
int main(void)
{
  ...
  for (i = 0; i < NTHREADS; ++i)
    if (pthread_join (threads[i], &res) != 0 || res != NULL)
      {
	fprintf(stderr,"thread %d returned a failure\n", i);
	result = 1;
      }
    else
