...
/* TODO: merge with version in mini_schedule_in
 */
static
int
nm_mini_make_ctrl_pkt(struct nm_sched		 *p_sched,
                      struct nm_mini_gate	 *p_mini_gate,
         ...
         */
        p_mini_ctrl->proto_id	= p_src_pw->proto_id;
        p_mini_ctrl->seq	= p_src_pw->seq;
...
nm_mini_out_schedule_gate(struct nm_gate *p_gate) {
        ...
        struct nm_mini_gate	*p_mini_gate	= NULL;
        ...
        p_mini_gate	= p_gate->sch_private;

        /* nothing to send, exit */
        if (!p_mini_gate->state
            &&
            !p_mini_gate->p_next_pw
            &&
        ...
        if (p_mini_gate->state == 0) {
                ...
                if (!p_mini_gate->p_next_pw)
                        p_mini_gate->p_next_pw	= tbx_slist_extract(pre);
                ...
                */
                 ...
                 */
                err = nm_mini_make_ctrl_pkt(p_gate->p_sched, p_mini_gate,
                                            p_mini_gate->p_next_pw, &p_pw);
                if (err != NM_ESUCCESS) {
                        NM_DISPF("mini_make_ctrl_pkt returned %d", err);
                }
                ...
                p_pw	= p_mini_gate->p_next_pw;
                p_pw->p_drv		= p_gdrv->p_drv;
                p_pw->p_trk		= p_gtrk->p_trk;
                p_pw->p_gdrv		= p_gdrv;
                p_pw->p_gtrk		= p_gtrk;
                p_mini_gate->p_next_pw	= NULL;
                p_mini_gate->state	= 0;
