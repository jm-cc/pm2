 ...
 * the target LWP.
 */
void ma_resched_task(marcel_t t LWPS_VAR_UNUSED, ma_lwp_t lwp)
{
	int vp;

	vp = ma_vpnum(lwp);

	PROF_EVENT1(sched_resched_task, t);
#ifdef MA__LWPS
	ma_preempt_disable();
	...
	if (! ma_test_tsk_thread_flag(t, TIF_POLLING_NRFLAG)) {
		PROF_EVENT2(sched_resched_lwp, ma_vpnum(MA_LWP_SELF), vp);
		MA_LWP_RESCHED(lwp);
	} else
		PROF_EVENT2(sched_resched_lwp_already_polling, t, vp);
...
static inline void try_to_resched(marcel_task_t *p, ma_holder_t *h)
{
	ma_lwp_t chosen;

	chosen = ma_find_lwp_for_task(p, h);
	if (tbx_likely(chosen)) {
	...
	if (ma_awake_task(p, state, &h)) {
		...
		if (rq && ma_rq_covers(rq, ma_vpnum(MA_LWP_SELF)) && TASK_PREEMPTS_TASK(p, MARCEL_SELF)) {
			...
			if (! sync) {/* only if we won't for sure yield() soon */
				...
				ma_resched_task(MARCEL_SELF, MA_LWP_SELF);
			}
		} else {
			try_to_resched(p, h);
			ma_holder_try_to_wake_up_and_unlock_softirq(h);
...
asmlinkage void ma_schedule_tail(marcel_task_t *prev)
{
	...
	if (tbx_unlikely(prev != __ma_get_lwp_var(idle_task) &&
			 MARCEL_SELF == __ma_get_lwp_var(idle_task)))
		ma_entering_idle();
#endif
	ma_preempt_enable();
}
...
#warning TODO: descendre dans les bulles ...
	...
	for (i = 0; i < MA_NR_VPS; i++) {
		lwp = ma_get_lwp_by_vpnum(i);
		if (tbx_likely(lwp))
			sum += ma_lwp_rq(lwp)->as_holder.nb_ready_entities;
	}
 ...
 * timeslices.
 */
void ma_scheduler_tick(int user_ticks, int sys_ticks TBX_UNUSED)
{
	int ticks;
	unsigned long long *counter;
	struct ma_lwp_usage_stat *lwpstat = &__ma_get_lwp_var(lwp_usage);

	/** do not have a system tick counter **/
	ticks = user_ticks ;
	...
	if (ma_softirq_count() - MA_SOFTIRQ_OFFSET) {
		counter = &lwpstat->softirq;
		goto ma_upd_counter;
	}
		
#ifdef MA__LWPS
	if (MARCEL_SELF == __ma_get_lwp_var(idle_task)) {
		if (marcel_vp_is_disabled(ma_vpnum(MA_LWP_SELF)))
			counter = &lwpstat->disabled;
		else
#else
        if (ma_currently_idle) {
#endif
		counter = &lwpstat->idle;
		goto ma_upd_counter;
	}

	if (SELF_GETMEM(as_entity.prio) >= MA_BATCH_PRIO)
		counter = &lwpstat->nice;
	else
		counter = &lwpstat->user;

ma_upd_counter:
	*counter += ticks;

	if (preemption_enabled() && ma_thread_preemptible()) {
		if (tbx_unlikely(ma_atomic_dec_and_test(&SELF_GETMEM(as_entity.time_slice)))) {
			ma_set_need_resched(1);
			MARCEL_SCHED_LOG("scheduler_tick: time slice expired\n");
			ma_atomic_set(&SELF_GETMEM(as_entity.time_slice),MARCEL_TASK_TIMESLICE);
		}
		...
		// verrouiller la bulle.
		{
			...
			if (tbx_likely(h) && ma_holder_type(h) != MA_RUNQUEUE_HOLDER) {
				b = ma_bubble_holder(h);
				if (tbx_unlikely(ma_atomic_dec_and_test(&b->as_entity.time_slice)))
					ma_bubble_tick(b);
			}
#endif
		}
...
static inline
marcel_t do_switch(marcel_t prev, marcel_t next, ma_holder_t *nexth)
{
#ifdef MARCEL_STATS_ENABLED
	/* update statistics */
	...
	ma_task_stats_set(long, prev, ma_stats_last_ran_offset, marcel_clock());
...
#ifdef MA__DEBUG
		if (tbx_likely(go_to_sleep && !go_to_sleep_traced)) {
...
restart:
...
#ifdef MA__BUBBLES
		if (ma_idle_scheduler_is_running() && vpnum >= 0) {
			if (ma_bubble_notify_idle_vp(vpnum))
				goto need_resched_atomic;
		}
#endif
...
#endif
	/** prev != NULL => do not take nexth lock 
	    if the next thread is thread is the previous one */
	if (cur == prev) {
		next = cur;
		goto switch_tasks;
	}
	...
	idx = ma_sched_find_first_bit(rq->active->bitmap);

#ifdef MA__LWPS
	if (rq != ma_lwp_rq(MA_LWP_SELF)) {
	...
	nextent = ma_queue_entry(ma_array_queue(rq->active, idx));
	
#ifdef MA__BUBBLES
	nextent = ma_bubble_sched(nextent, rq, &nexth, idx);
	if (tbx_unlikely(NULL == nextent)) {
...
switch_tasks:
...
#ifdef MA__DEBUG
	if (go_to_sleep && 
	    ((prev->state == MA_TASK_DEAD) || !(ma_preempt_count() & MA_PREEMPT_ACTIVE)))
	...
	if (prev != next) {
		...
		/* really switch */
		prev = do_switch(prev, next, nexth);
...
int marcel_yield_to(marcel_t next)
{
	...
	if (tbx_unlikely(! MA_TASK_IS_READY(next))) {
		ma_task_holder_unlock_softirq(nexth);
		MARCEL_SCHED_LOG("marcel_yield: %s task %p\n", 
				 MA_TASK_IS_RUNNING(next) ? "busy" : "not enqueued", 
				 next);
		MARCEL_LOG_RETURN(-1);
	}
...
{
	MA_BUG_ON(SELF_GETMEM(preempt_count) != irq);
	do {
		SELF_SETMEM(preempt_count, MA_PREEMPT_ACTIVE);
		...
		MA_BUG_ON(SELF_GETMEM(preempt_count) != MA_PREEMPT_ACTIVE);
		SELF_SETMEM(preempt_count, irq);
...
#endif
DEF_LIBC(int, sched_yield, (void),())
DEF___LIBC(int, sched_yield, (void),())


/* La définition n'est pas toujours dans pthread.h */
#if !HAVE_DECL_PTHREAD_YIELD
...
DEF_PMARCEL(int, getaffinity_np, (pmarcel_t pid LWPS_VAR_UNUSED, size_t cpusetsize TBX_UNUSED, pmarcel_cpu_set_t *cpuset), 
...
static void linux_sched_lwp_init(ma_lwp_t lwp LWPS_VAR_UNUSED)
{
	...
	{
		if (num >= (int)marcel_nbvps()) {
			marcel_snprintf(name,sizeof(name), "vp%d", num);
...
MA_DEFINE_LWP_NOTIFIER_START_PRIO(linux_sched, MA_NOTIFIER_PRIO_SCHED, "Linux scheduler",
				  linux_sched_lwp_init, "Initialisation des runqueue",
				  linux_sched_lwp_start, "Activation de la tache courante");
...
static void init_subrunqueues(struct marcel_topo_level *level, ma_runqueue_t *rq, int levelnum) {
	...
	for (i=0;i<level->arity;i++) {
		...
		if (level->children[i]->type == MARCEL_LEVEL_FAKE)
			marcel_snprintf(name, sizeof(name), "Fake%d-%u",
					levelnum, level->children[i]->number);
		else if (level->children[i]->type == MARCEL_LEVEL_MISC)
			marcel_snprintf(name, sizeof(name), "Misc%d-%u",
					levelnum, level->children[i]->number);
		else
#endif
			marcel_snprintf(name,sizeof(name), "%s%u",
...
{
	if (ma_preempt_count() - MA_SOFTIRQ_OFFSET > 1) {
#ifdef PROFILE
		do {
			if (ma_spin_is_locked(lock)) {
				PROF_EVENT(spin_lock_busy);
...
void __ma_preempt_read_lock(ma_rwlock_t *lock)
{
	if (ma_preempt_count() > 1) {
		...
		} while (!_ma_raw_read_trylock(lock));
#else
