...
#define NM_DCFA_NUIOA_SYSFILE_LENGTH 16
...
int nm_dcfa_hca_get_numa_node(struct nm_dcfa_hca_s*p_hca)
{
  ...
  FILE *sysfile = NULL;
  char file[128] = "";
  char line[NM_DCFA_NUIOA_SYSFILE_LENGTH]="";
  ...
  if (sysfile) {
    int node;
    fgets(line, NM_DCFA_NUIOA_SYSFILE_LENGTH, sysfile);
    fclose(sysfile);
...
#ifdef PM2_NUIOA
  p_profile->numa_node = nm_dcfa_hca_get_numa_node(p_hca);
#endif
