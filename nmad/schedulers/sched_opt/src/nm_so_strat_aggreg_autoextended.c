...
static int strat_aggreg_autoextended_pack(void*, struct nm_gate*, uint8_t, uint8_t, void*, uint32_t);
static int strat_aggreg_autoextended_pack_ctrl(void*, struct nm_gate *, union nm_so_generic_ctrl_header*);
static int strat_aggreg_autoextended_try_and_commit(void*, struct nm_gate*);
static int strat_aggreg_autoextended_rdv_accept(void*, struct nm_gate*, uint8_t*, uint8_t*);
static int strat_aggreg_autoextended_flush(void*, struct nm_gate*);

static const struct nm_so_strategy_driver nm_so_strat_aggreg_autoextended_driver =
  {
...
{
  puk_adapter_declare("Nm_strategy_aggreg_autoextended",
		      puk_adapter_provides("PadicoAdapter", &nm_so_strat_aggreg_autoextended_adapter_driver),
		      puk_adapter_provides("NmStrategy", &nm_so_strat_aggreg_autoextended_driver),
                                           ...
                                           struct nm_gate *p_gate)
{
  ...
  list_for_each_entry(p_so_pw, &status->out_list, link) {
    NM_SO_SR_TRACE("Marking pw %p as completed\n", p_so_pw);
    p_so_pw->is_completed = tbx_true;
  }
                                          ...
                                          void *data, uint32_t len)
{
  ...
  if(len <= status->nm_so_max_small) {
    NM_SO_SR_TRACE("Small packet\n");

    NM_SO_SR_TRACE("We first try to find an existing packet to form an aggregate\n");
    list_for_each_entry(p_so_pw, &status->out_list, link) {
    ...
    NM_SO_SR_TRACE("First allocate a packet wrapper\n");
    err = nm_so_pw_alloc_and_fill_with_data(tag + 128, seq,
                                            data, len,
					    0, 1,
    ...
    NM_SO_SR_TRACE("Then place it into the appropriate list of large pending sends.\n");
                                                    ...
                                                    struct nm_gate *p_gate)
{
  ...
  if(p_so_pw->is_completed == tbx_true) {
    NM_SO_SR_TRACE("pw %p is completed\n", p_so_pw);
