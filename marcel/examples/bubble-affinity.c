...
/* This experimental program is an attempt to provide an automatic test
   framework for bubble scheduler.  The idea is to generate topologies and
   bubble hierarchies, pass them to a bubble scheduler's `submit ()' method,
   and check whether the resulting scheduling entity distribution matches our
   expectations.  */
...
{
  /* An example topology: 2 nodes, each of which has 2 CPUs, each of which has
     4 cores.  */
