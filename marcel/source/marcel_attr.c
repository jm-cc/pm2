...
DEF_MARCEL_PMARCEL(int, attr_setstacksize, (marcel_attr_t *attr, size_t stacksize), (attr, stacksize),
{
	MARCEL_LOG_IN();

	if (stacksize < sizeof(marcel_task_t)) {
		MA_WARN_USER("(p)marcel_attr_setstacksize: stack size (%luB requested) must be greater than %luB\n",
			     stacksize, sizeof(marcel_task_t));
		MARCEL_LOG_RETURN(EINVAL);
	}
	if (stacksize > THREAD_SLOT_SIZE) {
		MA_WARN_USER("(p)marcel_attr_setstacksize: requested %uB but `THREAD_SLOT_SIZE' is only %luB\n",
			     (unsigned int) stacksize, THREAD_SLOT_SIZE);
		MARCEL_LOG_RETURN(EINVAL);
	}

	if (attr->__stackaddr)
		attr->__stackaddr = (char *) attr->__stackaddr + stacksize - attr->__stacksize;
...
DEF_PMARCEL(int, attr_setstackaddr, (marcel_attr_t *attr, void *addr), (attr, addr),
{
	...
	if ((unsigned long)addr >= (1ul<<32)) {
		MA_WARN_USER("pthread_setstack: stack address must not be greater than %lu\n", 1ul<<32);
		MARCEL_LOG_RETURN(EINVAL);
	}
#endif
	    ...
	    (attr, stackaddr, stacksize),
{
	...
	if (stacksize < PMARCEL_STACK_MIN) {
		MA_WARN_USER("(p)marcel_setstack: stack size (%luB requested) must be greater than %luB\n",
			     stacksize, PMARCEL_STACK_MIN);
		MARCEL_LOG_RETURN(EINVAL);
	}
#ifndef MA__SELF_VAR
	if (stacksize > THREAD_SLOT_SIZE) {
		MA_WARN_USER("(p)marcel_setstack: requested %uB but `THREAD_SLOT_SIZE' is only %luB\n",
			     (unsigned int) stacksize, THREAD_SLOT_SIZE);
		MARCEL_LOG_RETURN(EINVAL);
	}

	if ((uintptr_t) ((char *)stackaddr + stacksize) % THREAD_SLOT_SIZE != 0) {
		MA_WARN_USER("pthread_setstack: stack is not aligned with THREAD_SLOT_SIZE\n");
		MARCEL_LOG_RETURN(EINVAL);
	}
#endif
...
static inline int check_attr_setdetachstate(int detached)
{
	...
	if ((detached != MARCEL_CREATE_DETACHED) && (detached != MARCEL_CREATE_JOINABLE)) {
		MARCEL_LOG("(p)marcel_attr_setdetachstate: invalid detached value\n");
		MARCEL_LOG_RETURN(EINVAL);
	}
	MARCEL_LOG_RETURN(0);
}
...
{
	MARCEL_LOG_IN();
	*detached = (attr->__flags & MA_ATTR_FLAG_DETACHSTATE) ? MARCEL_CREATE_DETACHED : MARCEL_CREATE_JOINABLE;
	MARCEL_LOG_RETURN(0);
})
...
DEF_PMARCEL(int, attr_setaffinity_np, (pmarcel_attr_t *attr, size_t cpusetsize TBX_UNUSED, const pmarcel_cpu_set_t *cpuset),
...
int lpt_attr_setaffinity_np(lpt_attr_t *__attr, size_t cpusetsize, const cpu_set_t *cpuset)
{
	...
	if (attr->__cpuset)
		...
		attr->__cpuset = vpset = marcel_malloc(sizeof(marcel_vpset_t));
...
int marcel_attr_getvpset(__const marcel_attr_t * __restrict attr, marcel_vpset_t * __restrict vpset)
{
	*vpset = attr->vpset;
	return 0;
}

DEF_PMARCEL(int, attr_getaffinity_np, (const pmarcel_attr_t *attr, size_t cpusetsize TBX_UNUSED, pmarcel_cpu_set_t *cpuset),
	    (attr, cpusetsize, cpuset),
{
...
int marcel_attr_settopo_level_on_core(marcel_attr_t * attr, int core)
{
        return marcel_attr_settopo_level(attr, &marcel_topo_core_level[core]);
}
...
int marcel_attr_setname(marcel_attr_t * __restrict attr, const char *__restrict name)
{
...
int marcel_attr_getname(__const marcel_attr_t * __restrict attr, char *__restrict name, size_t n)
{
...
int marcel_attr_getpreemptible(__const marcel_attr_t * __restrict attr, int *__restrict preemptible)
{
...
int marcel_attr_getflags(__const marcel_attr_t * __restrict attr, int *__restrict flags)
{
	*flags = attr->flags;
	return 0;
}
...
int marcel_attr_getseed(__const marcel_attr_t * __restrict attr, int * __restrict seed)
{
	*seed = attr->seed;
	return 0;
}
...
DEF_PMARCEL(int,attr_setinheritsched,(pmarcel_attr_t * attr,int inheritsched),(attr,inheritsched),
{
	...
	if ((inheritsched != PMARCEL_INHERIT_SCHED)
		...
		MARCEL_LOG("pmarcel_attr_setinheritsched : valeur inheritsched (%d) invalide\n",
			   inheritsched);
		MARCEL_LOG_RETURN(EINVAL);
	}

	if (PMARCEL_INHERIT_SCHED == inheritsched)
...
DEF_PMARCEL(int,attr_getinheritsched,(__const pmarcel_attr_t * __restrict attr,int * __restrict inheritsched),(attr,inheritsched),
{
	MARCEL_LOG_IN();
	if (!attr) {
		MARCEL_LOG("pthread_attr_getinheritsched : attr NULL ou adresse policy invalide !\n");
		MARCEL_LOG_RETURN(EINVAL);
	}

	*inheritsched = (attr->__flags & MA_ATTR_FLAG_INHERITSCHED) ? PMARCEL_INHERIT_SCHED : PMARCEL_EXPLICIT_SCHED;
	MARCEL_LOG_RETURN(0);
})
...
/***************************get/setscope**************************/
DEF_MARCEL_PMARCEL(int,attr_setscope,(marcel_attr_t *attr, int contentionscope),(attr,contentionscope),
{
	MARCEL_LOG_IN();
...
DEF_MARCEL_PMARCEL(int,attr_getscope,(__const marcel_attr_t *__restrict attr, int *contentionscope),(attr,contentionscope),
{
	/* error checking */
	if (!attr) {
		MARCEL_LOG("pmarcel_attr_setscope : attr NULL !\n");
...
{
	struct marcel_sched_param sched_param;
	...
	if ((policy != SCHED_FIFO) && (policy != SCHED_RR) && (policy != SCHED_OTHER)) {
		MARCEL_LOG("pthread_attr_setschedpolicy : police d'ordonnancement invalide!\n");
		MARCEL_LOG_RETURN(EINVAL);
	}
	
	/* convert marcel priority according to the new policy */
	pmarcel_attr_getschedparam(attr, &sched_param);
	attr->__schedpolicy = policy;
	pmarcel_attr_setschedparam(attr, &sched_param);
...
DEF_PMARCEL(int,attr_setschedparam,(pmarcel_attr_t * __restrict attr, __const struct marcel_sched_param * __restrict param),(attr,param),
{
	...
	if ((policy == SCHED_RR) || (policy == SCHED_FIFO)) {
		if ((param->__sched_priority >= 0) && (param->__sched_priority <= MA_RT_PRIO))
...
DEF_PMARCEL(int,attr_getschedparam,(__const pmarcel_attr_t *__restrict attr, struct marcel_sched_param *__restrict param),(attr,param),
{
	...
	ret = pmarcel_attr_getschedpolicy(attr, &policy);
	if (ret == EINVAL) {
		MARCEL_LOG("pmarcel_attr_getschedpolicy retourne une police invalide\n");
		MARCEL_LOG_RETURN(EINVAL);
	}
...
int lpt_getattr_np(lpt_t t,lpt_attr_t *__attr)
{
	...
	attr->__stackaddr = (char *)attr->__stackaddr + attr->__stacksize;
	if (! attr->__cpuset)
	        attr->__cpuset = marcel_malloc(sizeof(marcel_vpset_t));
