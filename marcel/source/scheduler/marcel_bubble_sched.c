...
/** retrieve some properties from the bubble class designed by klass **/
/**
...
int marcel_bubble_init(marcel_bubble_t * bubble)
{
	...
	*bubble = (marcel_bubble_t) MARCEL_BUBBLE_INITIALIZER(*bubble);
	...
	marcel_snprintf(bubble->as_holder.name, MARCEL_MAXNAMESIZE, "bubble_h %p", bubble);
	marcel_snprintf(bubble->as_entity.name, MARCEL_MAXNAMESIZE, "bubble_e %p", bubble);
...
int marcel_bubble_entity_set_settled(marcel_entity_t *e, int is_settled)
{
	ma_holder_t *h = ma_entity_holder_lock_softirq(e);
	e->settled = is_settled;
	ma_entity_holder_unlock_softirq(h);
	return 0;
}

/* Check if entity is settled on the current runqueue */
int marcel_bubble_entity_is_settled(marcel_entity_t *e)
{
	int is_settled;
	ma_holder_t *h;
	
	h = ma_entity_holder_lock_softirq(e);
	is_settled = e->settled;
	ma_entity_holder_unlock_softirq(h);
	return is_settled;
}

/* Marks the bubble as settled on the current runqueue. Used by some
   bubble schedulers. (can be 0 or 1) */
int marcel_bubble_set_settled(marcel_bubble_t *b, int is_settled)
{
	return marcel_bubble_entity_set_settled(&b->as_entity, is_settled);
}

/* Check if bubble is settled on the current runqueue */
int marcel_bubble_is_settled(marcel_bubble_t *b)
{
	return marcel_bubble_entity_is_settled(&b->as_entity);
}

/* Marks the task as settled on the current runqueue. Used by some
   bubble schedulers. (can be 0 or 1) */
int marcel_bubble_task_set_settled(marcel_task_t *t, int is_settled)
{
	return marcel_bubble_entity_set_settled(&t->as_entity, is_settled);
}

/* Check if task is settled on the current runqueue */
int marcel_bubble_task_is_settled(marcel_task_t *t)
{
	return marcel_bubble_entity_is_settled(&t->as_entity);
}
...
{
        return marcel_bubble_insertentity(bubble, &little_bubble->as_entity);
}

/** Lazy bring the task \e task inside the bubble \e bubble by
 * setting its sched_holder and natural_holder accordingly. */
int marcel_bubble_inserttask(marcel_bubble_t * bubble, marcel_task_t * task)
{
        return marcel_bubble_insertentity(bubble, &task->as_entity);
}

/** Move task inside the bubble */
int marcel_bubble_inserttask_move(marcel_bubble_t * bubble, marcel_task_t * task)
{
	// TODO: affiner verrouillage, ne pas utiliser ma_move_entity

	marcel_bubble_t *parent_bubble;
	ma_runqueue_t *rq;

	parent_bubble = marcel_bubble_holding_task(task);
	rq = ma_get_parent_rq(&parent_bubble->as_entity);

	/* Make the thread part of the bubble. */
        marcel_bubble_inserttask(bubble, task);

	/* Physically move the thread inside the bubble */
	ma_topo_lock_all (rq->topolevel);
	ma_move_entity (&task->as_entity, &bubble->as_holder);
	ma_topo_unlock_all (rq->topolevel);

	return 0;
}

/** Move task inside the bubble but keep previous runqueue affinity */
int marcel_bubble_inserttask_stay(marcel_bubble_t * bubble, marcel_task_t * task)
{
	// TODO: affiner verrouillage, ne pas utiliser ma_move_entity

	marcel_bubble_t *holding_bubble;
	ma_runqueue_t *prev_rq, *top_rq;

	holding_bubble = marcel_bubble_holding_task(task);
	prev_rq = ma_get_parent_rq(&holding_bubble->as_entity);
	top_rq = ma_get_parent_rq(&bubble->as_entity);

	marcel_bubble_inserttask(bubble, task);

	if (top_rq != prev_rq) {
		struct marcel_topo_level *top_level;

		top_level = marcel_topo_common_ancestor (prev_rq->topolevel, top_rq->topolevel);
		ma_topo_lock_all (top_level);
		ma_move_entity (&task->as_entity, &prev_rq->as_holder);
...
{
        return marcel_bubble_removeentity(bubble, &little_bubble->as_entity);
}
...
{
        return marcel_bubble_removeentity(bubble, &task->as_entity);
}
...
int ma_bubble_detach(marcel_bubble_t * b)
{
	...
	marcel_bubble_t *hb = ma_bubble_holder(h);
	MA_BUG_ON(ma_holder_type(hb->as_entity.sched_holder) != MA_RUNQUEUE_HOLDER);
#endif
...
static ma_runqueue_t *push_up_to(ma_holder_t * h)
{
	...
	if (h != NULL && h->type == MA_RUNQUEUE_HOLDER) {
		...
		while (target && marcel_vpset_isincluded(&marcel_disabled_vpset, &target->vpset))
...
void ma_push_entities(const marcel_vpset_t * vpset TBX_UNUSED)
{
...
const marcel_bubble_sched_t *marcel_lookup_bubble_scheduler(const char *name)
{
	...
	if (klass) {
		int err;
		scheduler = marcel_malloc(klass->instance_size);
		if (scheduler) {
