...
int main(int argc, char**argv){
  ...
  int sbuf[BUFF_SIZE], rbuf[BUFF_SIZE];
  MPI_Aint size, lb, ub;
  ...
  MPI_Get_address(&sbuf[0], &lb);
  MPI_Get_address(&sbuf[BUFF_SIZE], &ub);
  size = ub - lb;
  ...
  MPI_Get_address(&sbuf[0], &lb);
  MPI_Get_address(&sbuf[2 * BUFF_SIDE + 2], &ub);
  ...
  for(i = 0; i < BUFF_SIZE; ++i)
    sbuf[i] = i;
  memset(rbuf, 0, sizeof(int) * BUFF_SIZE);

  MPI_Win_create(rbuf, size, disp_unit, MPI_INFO_NULL, MPI_COMM_WORLD, &win);

  sbuf[0] = 42;
  { // Single int
    MPI_Win_lock(MPI_LOCK_EXCLUSIVE, rank, 0, win);
    MPI_Put(sbuf, 1, MPI_INT, rank, 0, 1, MPI_INT, win);
    MPI_Win_unlock(rank, win);
  }

  print_arr(rbuf, BUFF_SIZE, BUFF_SIDE);
  MPI_Win_lock(MPI_LOCK_EXCLUSIVE, rank, 0, win);
  memset(rbuf, 0, sizeof(int) * BUFF_SIZE);
  MPI_Win_unlock(rank, win);
  ...
  { // Array of ints to array of int
    MPI_Win_lock(MPI_LOCK_EXCLUSIVE, rank, 0, win);
    MPI_Put(sbuf, 1, mpi_arr_t, rank, 0, BUFF_SIZE, MPI_INT, win);
    MPI_Win_unlock(rank, win);
  }
  ...
  { // Matrix diag to array of int
    MPI_Win_lock(MPI_LOCK_EXCLUSIVE, rank, 0, win);
    MPI_Put(sbuf, 1, mpi_diag_t, rank, 0, BUFF_SIDE, MPI_INT, win);
    MPI_Win_unlock(rank, win);
  }
  ...
  { // Matrix diag 2x2 blocks to array of int
    MPI_Win_lock(MPI_LOCK_EXCLUSIVE, rank, 0, win);
    MPI_Put(sbuf, BUFF_SIDE / 2, mpi_block_2_t, rank, 0, 2 * BUFF_SIDE, MPI_INT, win);
    MPI_Win_unlock(rank, win);
  }
  ...
  { // Matrix diag 2x2 blocks to array of int
    MPI_Win_lock(MPI_LOCK_EXCLUSIVE, rank, 0, win);
    MPI_Put(sbuf, 1, mpi_diag_block_2_t, rank, 0, 2 * BUFF_SIDE, MPI_INT, win);
    MPI_Win_unlock(rank, win);
  }
  ...
  { // Array of int to matrix diag
    MPI_Win_lock(MPI_LOCK_EXCLUSIVE, rank, 0, win);
    MPI_Put(sbuf, BUFF_SIDE, MPI_INT, rank, 0, 1, mpi_diag_t, win);
    MPI_Win_unlock(rank, win);
  }
  ...
  { // Array of int to matrix diag 2x2 blocks
    MPI_Win_lock(MPI_LOCK_EXCLUSIVE, rank, 0, win);
    MPI_Put(sbuf, 2 * BUFF_SIDE, MPI_INT, rank, 0, BUFF_SIDE / 2, mpi_block_2_t, win);
    MPI_Win_unlock(rank, win);
  }
  ...
  { // Array of int to matrix diag 2x2 blocks
    MPI_Win_lock(MPI_LOCK_EXCLUSIVE, rank, 0, win);
    MPI_Put(sbuf, 2 * BUFF_SIDE, MPI_INT, rank, 0, 1, mpi_diag_block_2_t, win);
    MPI_Win_unlock(rank, win);
  }
  ...
  { // Matrix diag 2x2 blocks to matrix diag 2x2 blocks
    MPI_Win_lock(MPI_LOCK_EXCLUSIVE, rank, 0, win);
    MPI_Put(sbuf, 1, mpi_diag_block_2_t, rank, 0, 1, mpi_diag_block_2_t, win);
    MPI_Win_unlock(rank, win);
  }
