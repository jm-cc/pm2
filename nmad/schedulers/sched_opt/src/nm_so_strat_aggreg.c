...
static int strat_aggreg_pack(void*, struct nm_gate*, uint8_t, uint8_t, void*, uint32_t);
static int strat_aggreg_packv(void*, struct nm_gate*, uint8_t, uint8_t, struct iovec *, int);
static int strat_aggreg_pack_ctrl(void*, struct nm_gate *, union nm_so_generic_ctrl_header*);
static int strat_aggreg_pack_ctrl_chunk(void*, struct nm_so_pkt_wrap *, union nm_so_generic_ctrl_header *);
static int strat_aggreg_pack_extended_ctrl(void*, struct nm_gate *, uint32_t, union nm_so_generic_ctrl_header *, struct nm_so_pkt_wrap **);
static int strat_aggreg_pack_extended_ctrl_end(void*,
                                                struct nm_gate *p_gate,
                                                struct nm_so_pkt_wrap *p_so_pw);
static int strat_aggreg_try_and_commit(void*, struct nm_gate*);
static int strat_aggreg_rdv_accept(void*, struct nm_gate*, uint8_t*, uint8_t*);

static const struct nm_so_strategy_driver nm_so_strat_aggreg_driver =
  {
...
extern int nm_so_strat_aggreg_init(void)
{
  puk_adapter_declare("Nm_strategy_aggreg",
		      puk_adapter_provides("PadicoAdapter", &nm_so_strat_aggreg_adapter_driver),
		      puk_adapter_provides("NmStrategy", &nm_so_strat_aggreg_driver),
