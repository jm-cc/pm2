...
static void mpi_bench_put_ac_server(void*buf, size_t len)
{
...
static void mpi_bench_put_ac_client(void*buf, size_t len)
{
  MPI_Win_post(grp_other, 0, win);
  MPI_Win_wait(win);
  MPI_Win_start(grp_peer, 0, win);
...
static void mpi_bench_put_ac_init(void*buf, size_t len, int count)
{
...
static void mpi_bench_put_ac_finalize(void)
{
...
const struct mpi_bench_s mpi_bench_rma_put_active =
  {
    .label    = "mpi_bench_rma_put_active",
    .name     = "MPI RMA Put Active",
    .server   = &mpi_bench_put_ac_server,
    .client   = &mpi_bench_put_ac_client,
    .init     = &mpi_bench_put_ac_init,
    .finalize = &mpi_bench_put_ac_finalize
  };
