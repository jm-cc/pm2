...
#endif
#define MAX_PACKS		8
#define NB_PACKS_OFFSET		8
#define MAX_MSGS		8
#define NB_MSGS_OFFSET		8
...
tester_thread(void *_arg)
{
	...
	LDISP("#### [%d]tester thread: %p", process_rank, marcel_self());

	madeleine	= arg->madeleine;
	name		= arg->name;
	...
	char **argv)
{
	...
	{
		p_tbx_slist_t thread_slist = NULL;

		thread_slist	= tbx_slist_nil();

		tbx_slist_ref_to_head(slist);
		do
		{
			p_tester_t tester = NULL;

			tester = TBX_MALLOC(sizeof(tester_t));

			tester->name = tbx_slist_ref_get(slist);
			tester->madeleine	= madeleine;
			tester->thread		= NULL;

			marcel_create(&(tester->thread), NULL, tester_thread, tester);
			tbx_slist_append(thread_slist, tester);
		}
		while (tbx_slist_ref_forward(slist));

		while (!tbx_slist_is_nil(thread_slist)) {
			p_tester_t tester	= NULL;

			tester = tbx_slist_extract(thread_slist);
