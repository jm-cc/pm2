...
DEF_MARCEL_PMARCEL(int, spin_init, (marcel_spinlock_t * lock, int pshared TBX_UNUSED),
		   (lock, pshared), 
{
	if ((pshared != MARCEL_PROCESS_SHARED) && (pshared != MARCEL_PROCESS_PRIVATE)) {
		MARCEL_LOG("(p)marcel_spin_init : valeur pshared(%d) invalide", pshared); 
		return EINVAL;
	}

	ma_spin_lock_init(&lock->lock); return 0;
})
 ...
 */
DEF_MARCEL_PMARCEL(int, spin_destroy, (marcel_spinlock_t * lock), (lock), 
{
...
DEF_MARCEL_PMARCEL(int, spin_lock, (marcel_spinlock_t * lock), (lock), 
{
...
DEF_MARCEL_PMARCEL(int, spin_trylock, (marcel_spinlock_t * lock), (lock), 
{
	ma_some_thread_preemption_disable(ma_self());
...
DEF_MARCEL_PMARCEL(int, spin_unlock, (marcel_spinlock_t * lock), (lock), 
{
