...
extern void _dl_get_tls_static_info(size_t * sizep,
				    size_t * alignp) ma_libc_internal_function;
...
static void *mapped_slot_alloc(void *foo TBX_UNUSED)
{
	...
	if (!ptr) {
		if (!ma_in_atomic() && nb_try_left--) {
			MARCEL_ALLOC_LOG
			    ("Not enough room for new stack (stack size is %lx, stack allocation area is %lx-%lx), trying to wait for other threads to terminate.\n",
			     THREAD_SLOT_SIZE, (unsigned long) SLOT_AREA_BOTTOM,
			...
			fprintf(stderr,
				"Not enough room for new stack (stack size is %lx, stack allocation area is %lx-%lx).\n",
				THREAD_SLOT_SIZE, (unsigned long) SLOT_AREA_BOTTOM,
				(unsigned long) ISOADDR_AREA_TOP);
	...
	res = mmap(ptr, THREAD_SLOT_SIZE,
		   PROT_READ | PROT_WRITE | PROT_EXEC, MMAP_MASK, FILE_TO_MAP, 0);

	if (res == MAP_FAILED) {
		if (!ma_in_atomic() && nb_try_left--) {
			MARCEL_ALLOC_LOG
			    ("mmap(%p, %lx, ...) for stack failed! Trying to wait for other threads to terminate\n",
			     next_slot, THREAD_SLOT_SIZE);
			marcel_yield();
			goto retry;
		}
		perror("mmap");
		fprintf(stderr, "args %p, %lx, %d, %d",
			ptr, THREAD_SLOT_SIZE, MMAP_MASK, FILE_TO_MAP);
		MARCEL_EXCEPTION_RAISE(MARCEL_CONSTRAINT_ERROR);
	}
...
void marcel_tls_attach(marcel_t t)
{
	...
	if (ret != 0) {
		fprintf(stderr, "entry #%u address %lx failed: %s", desc.entry_number,
...
static void marcel_slot_init(void)
{
	...
	if (static_tls_size + sizeof(lpt_tcb_t) > MA_TLS_AREA_SIZE) {
		fprintf(stderr,
			"Marcel has only %lu bytes for TLS while %lu are needed, please increase MA_TLS_AREA_SIZE in marcel_config.h. Aborting.\n",
			(unsigned long) MA_TLS_AREA_SIZE,
	...
	asm("movl %0, %%gs:(%c1)"::"r"(1),
	    "i"(tbx_offset_of(lpt_tcb_t, multiple_threads)));
      asm("movl %%gs:(%c1), %0": "=r"(sysinfo):"i"(tbx_offset_of
	    (lpt_tcb_t, sysinfo)));
      asm("movl %%gs:(%c1), %0": "=r"(stack_guard):"i"(tbx_offset_of
	    (lpt_tcb_t,
	     stack_guard)));
      asm("movl %%gs:(%c1), %0": "=r"(pointer_guard):"i"(tbx_offset_of
	    (lpt_tcb_t,
	     pointer_guard)));
      asm("movl %%gs:(%c1), %0": "=r"(gscope_flag):"i"(tbx_offset_of
	    (lpt_tcb_t,
	     gscope_flag)));
      asm("movl %%gs:(%c1), %0": "=r"(private_futex):"i"(tbx_offset_of
	    (lpt_tcb_t,
	     private_futex)));
#elif defined(X86_64_ARCH)
	syscall(SYS_arch_prctl, ARCH_GET_FS, &__main_thread_tls_base);
	__main_thread->tls_desc = 0;
	asm("movl %0, %%fs:(%c1)"::"r"(1),
	    "i"(tbx_offset_of(lpt_tcb_t, multiple_threads)));
      asm("movl %%fs:(%c1), %0": "=r"(gscope_flag):"i"(tbx_offset_of
	    (lpt_tcb_t,
	     gscope_flag)));
      asm("movq %%fs:(%c1), %0": "=r"(sysinfo):"i"(tbx_offset_of
	    (lpt_tcb_t, sysinfo)));
      asm("movq %%fs:(%c1), %0": "=r"(stack_guard):"i"(tbx_offset_of
	    (lpt_tcb_t,
	     stack_guard)));
      asm("movq %%fs:(%c1), %0": "=r"(pointer_guard):"i"(tbx_offset_of
	    (lpt_tcb_t,
	     pointer_guard)));
      asm("movl %%fs:(%c1), %0": "=r"(private_futex):"i"(tbx_offset_of
	    (lpt_tcb_t,
	     private_futex)));
...
#endif
	marcel_tls_slot_allocator =
	    ma_new_obj_allocator(0, tls_slot_alloc, NULL, tls_slot_free, NULL,
				 POLICY_HIERARCHICAL_MEMORY, thread_cache_max);
#endif
...
void marcel_see_allocated_memory(marcel_entity_t * entity)
{
	...
	for (i = 0; i < marcel_nbnodes; i++)
		marcel_fprintf(stderr,
			       "ma_see_allocated_memory : thread %p : node %d -> %d Po\n",
