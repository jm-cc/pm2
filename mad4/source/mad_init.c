...
compute_mtu(p_mad_directory_t          dir,
  ...
  if (med == dst){
    ...
    if (dedicated_fchannel){
      ...
      fchannel = tbx_htable_get(dir->channel_htable, cdata->channel_name);
      channel  = tbx_htable_get(dir->channel_htable, fchannel->cloned_channel_name);
    }else{
      channel = tbx_htable_get(dir->channel_htable, cdata->channel_name);
    }
...
static p_mad_dir_connection_data_t
...
mad_dir_channel_init(p_mad_madeleine_t madeleine){
  ...
  /****/
  // Construction of the channel table with the htable
  {
