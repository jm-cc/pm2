...
static void ma_memory_print_affinities(marcel_bubble_t * bubble)
{
	...
	marcel_fprintf(stderr, "Entity %p (%s): [ ", e,
	...
	for (i = 0; i < marcel_nbnodes; i++) {
		marcel_fprintf(stderr, "%8ld%s", nodes[i],
			       (i == (marcel_nbnodes - 1)) ? " ]\n" : ", ");
	}
				...
				tbx_bool_t work_stealing)
{
	...
	cache_sched = marcel_malloc(marcel_bubble_sched_instance_size(&marcel_bubble_cache_sched_class));
	if (cache_sched) {
...
{
	marcel_printf("WARNING: You're currently trying to use the memory bubble scheduler, but Marcel was not configured with --enable-mami. The bubble scheduler will not do anything !");
	return 0;
}
