...
PADICO_MODULE_COMPONENT(NewMad_Driver_tcp,
  puk_component_declare("NewMad_Driver_tcp",
			puk_component_provides("PadicoComponent", "component", &nm_tcp_component_driver),
 ...
 *  @return The NM status code.
 */
extern int nm_tcp_exit(nm_drv_t p_drv)
{
  struct nm_tcp_drv *p_tcp_drv = p_drv->priv;
...
static int nm_tcp_poll_in(void*_status, struct nm_pkt_wrap *p_pw, int timeout)
{
  ...
  active_gate_found:
    ...
    NM_TRACE_PTR("gateless request: active gate id ", p_pw->p_gate);
  }
  ...
  if (p_tcp_trk->nb_incoming) {
    ...
    NM_TRACE_PTR("tcp incoming single poll: checking gate id", p_gate);
...
nm_tcp_send 	(void*_status,
        ...
        write_body:
                ...
                NM_TRACE_PTR("tcp outgoing gate id", p_pw->p_gate);
        ...
        writev_again:
                ...
                NM_TRACE_PTR("tcp header outgoing gate id", p_pw->p_gate);
        write_again:
#ifdef MSG_MORE
...
static int nm_tcp_recv(void*_status, struct nm_pkt_wrap *p_pw, int timeout)
{
  ...
  read_body:
    ...
    NM_TRACE_PTR("tcp incoming gate id", p_pw->p_gate);
  ...
  read_body_again:
    ...
    *p_cur = p_vi->cur_copy;
    ...
    NM_TRACE_PTR("tcp header incoming gate id", p_pw->p_gate);
  read_header_again:
