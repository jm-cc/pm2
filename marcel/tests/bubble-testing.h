...
/* bubbles: avoid malloc() */
#define BUBBLE_MAX_CHILDREN 1000
static marcel_bubble_t bubble_pool[BUBBLE_MAX_CHILDREN];
static int bubble_pool_index = 0;
	   ...
	   const char *format, ...)
{
	...
	marcel_snprintf (prefix + indent, sizeof (prefix) - indent - 1,
			 "[%2u] ", level->number);
	...
	if (out == NULL)
	{
		...
		marcel_vsnprintf (messages[message_count++], sizeof (messages[0]),
				  long_format, args);
	}
	else
			   ...
			   ma_atomic_t *thread_exit_signal)
{
	...
	if (*level_breadth > 0)
	{
		if (* (level_breadth + 1) == 0)
		{
			...
			marcel_attr_setnaturalbubble (&attr, bubble);
		...
		else
		{
			...
			{
				if (bubble_pool_index < BUBBLE_MAX_CHILDREN) {
					child = bubble_pool + bubble_pool_index;
					bubble_pool_index ++;
				} else
					child = NULL;
...
free_bubble_hierarchy (void)
{
	...
	for (i = 0; i < bubble_pool_index; i ++)
		marcel_bubble_join(bubble_pool + i);
}
...
test_marcel_bubble_scheduler (int argc, char *argv[],
			      const marcel_bubble_sched_class_t *scheduler_class,
			      ...
			      const tree_t *expected_result)
{
	...
	tbx_topology_init(argc, new_argv) ;
	...
	make_simple_bubble_hierarchy (bubble_hierarchy_description, 1,
				      &thread_exit_signal);
	...
	if (! matches_p)
		fprintf (stderr, "scheduling entities were NOT distributed as expected\n");
#ifdef VERBOSE_TESTS
	else
		fprintf (stderr,"scheduling entities were distributed as expected\n");
#endif
...
#endif /* ! MARCEL_PTHREAD && MARCEL_BUBBLES */
