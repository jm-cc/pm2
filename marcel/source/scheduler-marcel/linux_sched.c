...
//				p->activated = -1;
			...
			/* Attention ici: h peut �re une bulle, auquel cas
			 * activate_task peut l�her la bulle pour verrouiller
			 * une runqueue */
...
//	ma_set_task_lwp(p, LWP_SELF);
	...
	/* il est possible de d�arrer sur une autre rq que celle de SELF,
...
//	}
	...
	// on donne la main aussit�, bien souvent le meilleur choix
	...
	if (ma_holder_type(h) == MA_RUNQUEUE_HOLDER && !ma_in_atomic()
		...
		/* XXX: on pourrait �re pr�mpt�entre-temps... */
...
int ma_sched_change_prio(marcel_t t, int prio) {
	 ...
	 * vouloir d�erouiller la bulle pour pouvoir verrouiller la runqueue */
...
// TODO Pour l'instant, on n'a pas de notion de tick syst�e.
...
#undef sys_ticks
	...
	//				rq->best_expired_prio = p->static_prio;
		...
		// attention: rq->lock ne doit pas �re pris pour pouvoir
...
need_resched_atomic:
	...
	if (prev->sched.state &&
			/* garde-fou pour �iter de s'endormir
			 * par simple pr�mption */
...
#ifdef PM2_DEV
#warning TODO: demander �l application de rebalancer
#endif
//		load_balance(rq, 1, cpu_to_node_mask(smp_processor_id()));
	...
	if (!(nextent = ma_bubble_sched(nextent, prevh, rq, &nexth, idx)))
		/* ma_bubble_sched aura lib��prevrq */
		goto need_resched_atomic;
#endif
...
//Pour quand on voudra ce m�anisme...
...
// Effectue un changement de contexte + �entuellement ex�ute des
...
/* La d�inition n'est pas toujours dans pthread.h */
extern int pthread_yield (void) __THROW;
DEF_PTHREAD_STRONG(int, yield, (void), ())

// Modifie le 'vpmask' du thread courant. Le cas �h�nt, il faut donc
...
// pour se d�lacer sur le LWP courant (cf terminaison des threads)
...
MA_DEFINE_LWP_NOTIFIER_START_PRIO(linux_sched, 200, "Linux scheduler",
				  ...
				  linux_sched_lwp_start, "Activation de la t�he courante");
