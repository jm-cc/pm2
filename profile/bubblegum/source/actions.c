...
void ExecuterFlash(GtkWidget *widget, gpointer data)
{
   ...
   gtk_main_iteration_do(FALSE);
   char command[1024];
   snprintf(command,sizeof(command),"( make "GENEC_NAME" ; pm2load "GENEC_NAME" --marcel-nvp 4 --marcel-maxarity 2 ; bubbles -d /tmp/prof_file_user_%s ; realplay autobulles.swf ) &",getenv("USER"));
   system(command);
   gtk_progress_bar_set_fraction(GTK_PROGRESS_BAR(progress_bar), 1);
   gtk_main_iteration_do(FALSE);
   gtk_widget_destroy(dialog);
