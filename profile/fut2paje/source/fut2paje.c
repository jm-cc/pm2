...
paje_output_file_init (void)
{
  ...
  fprintf (out_paje_file, "2       ST_EvState     CT_VP           \"Ev State\"\n");
  ...
  fprintf (out_paje_file, "6       uw             ST_VPState      UserWaiting    \"0.0 0.0 0.0\"\n");
  fprintf (out_paje_file, "6       f              ST_VPState      ForestGOMP     \".9 0.0 0.0\"\n");
  fprintf (out_paje_file, "6       fw             ST_VPState      ForestGOMPWaiting     \"0.0 0.0 0.0\"\n");
  fprintf (out_paje_file, "6       e1             ST_EvState      Event1     \"0.0 0.0 0.8\"\n");
}
...
static int
handle_fgomp_wait_on (void)
{
  int thread_id = find_thread_id (ev.param[0]);

  fprintf (events_file, "10       %f	ST_VPState     VP%lu      %s      \"%s\"\n",
	   (float)((ev.time-start_time)/1000000.0),
	   ev.param[1],
	   thread_name[thread_id][0] != 'm' ? "fw" : "uw",
	   thread_name[thread_id]);
...
static int
handle_fgomp_get_wtime (void)
{
  int thread_id = find_thread_id (ev.param[0]);

  fprintf (events_file, "21       %f	ST_EvState     VP%lu      e1      \"get_wtime\"      \"%s\"\n",
	   (float)((ev.time-start_time)/1000000.0),
	   ev.param[1],
	   thread_name[thread_id]);
...
/* 	case FUT_FGOMP_WAIT_ON_CODE: */
/* 	  handle_fgomp_wait_on (); */
/* 	  break; */

/* 	case FUT_FGOMP_WAIT_OFF_CODE: */
/* 	  handle_fgomp_wait_off (); */
/* 	  break; */

/* 	case FUT_FGOMP_GET_WTIME_CODE: */
/* 	  handle_fgomp_get_wtime (); */
/* 	  break; */
...
write_paje_header (FILE *file)
{
  ...
  fprintf (file, "\%%	Key	string\n");
  fprintf (file, "\%%EndEventDef\n");
  fprintf (file, "\%%EventDef	PajeNewEvent   20\n");
  fprintf (file, "\%%	Time	date\n");
  fprintf (file, "\%%	Type	string\n");
  fprintf (file, "\%%	Container	string\n");
  fprintf (file, "\%%	Value	string\n");
  ...
  fprintf (file, "\%%EndEventDef\n");
  fprintf (file, "\%%EventDef	PajeNewEvent   21\n");
  fprintf (file, "\%%	Time	date\n");
  fprintf (file, "\%%	Type	string\n");
  fprintf (file, "\%%	Container	string\n");
  fprintf (file, "\%%	Value	string\n");
