...
do { \
        if (ma_preempt_count()==irq*MA_HARDIRQ_OFFSET && ma_thread_preemptible() && tbx_unlikely(ma_need_resched())) \
...
/* Protection pour la r�ntrance du code de marcel avec les
 * biblioth�ues ext�ieures (glibc par exemple)
 * D�R�I� utilisez marcel_extlib_un?protect()
 */
