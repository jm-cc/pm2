...
#define NM_SO_POLICY_RATE_PACKS 0
#define NM_SO_POLICY_RATE_CTRLS 1

struct nm_so_policy_rate_gate{
  struct tbx_fast_list_head out_list[2];
  struct tbx_fast_list_head pack_ctrl_out_list;  
  unsigned nb_packets;
  unsigned round;
};
	  ...
	  union nm_so_generic_ctrl_header *p_ctrl)
{
  struct tbx_fast_list_head *out_list =
    &((struct nm_so_policy_rate_gate *)private)->out_list[NM_SO_POLICY_RATE_CTRLS];
  ...
  ((struct nm_so_policy_rate_gate *)private)->nb_packets++;
     ...
     void *data, uint32_t len)
{
  ...
  if(len <= NM_SO_MAX_SMALL) {
    ...
    struct tbx_fast_list_head *out_list =
      &((struct nm_so_policy_rate_gate *)private)->out_list[NM_SO_POLICY_RATE_PACKS];
...
static int switch_list(struct nm_so_policy_rate_gate *p_so_lg, struct tbx_fast_list_head **p_out_list)
{
  if(*p_out_list == &(p_so_lg->out_list[NM_SO_POLICY_RATE_CTRLS]))
    *p_out_list = &(p_so_lg->out_list[NM_SO_POLICY_RATE_PACKS]);
  else
    *p_out_list = &(p_so_lg->out_list[NM_SO_POLICY_RATE_CTRLS]);
...
{
  struct nm_so_policy_rate_gate *p_so_rg = ((struct nm_so_policy_rate_gate *)private);
  struct tbx_fast_list_head *out_list;
  struct nm_pkt_wrap *p_so_pw;
  unsigned *round = &p_so_rg->round;

  if(++(*round) == 10)
    {
      out_list = &p_so_rg->out_list[NM_SO_POLICY_RATE_PACKS];
      *round = 0;
    }
  else
    out_list = &p_so_rg->out_list[NM_SO_POLICY_RATE_CTRLS];
  
  if(tbx_fast_list_empty(out_list))
    {
      switch_list(p_so_rg, &out_list);
...
{
  struct nm_so_policy_rate_gate *priv = 
    TBX_MALLOC(sizeof(struct nm_so_policy_rate_gate));

  TBX_INIT_FAST_LIST_HEAD(&priv->out_list[NM_SO_POLICY_RATE_PACKS]); 
  TBX_INIT_FAST_LIST_HEAD(&priv->out_list[NM_SO_POLICY_RATE_CTRLS]); 
  priv->nb_packets = 0;
  priv->round = 0;
...
{
  struct nm_so_policy_rate_gate *priv = addr_private[0];
...
busy(void *private)
{
  return ((struct nm_so_policy_rate_gate *)private)->nb_packets;
}

nm_so_policy nm_so_policy_rate = {
  .pack_ctrl = pack_ctrl,
  .pack = pack,
