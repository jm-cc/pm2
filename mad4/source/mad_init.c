             ...
             p_tbx_htable_t dir_adapter_htable)
{
  ...
  /***/
  // -> Détermination du nombre de ports d'émission
  // -> ouverture des ports
  // mise en place des structures d'accès aux zones à émettre
  mad_adapter->s_track_set = NULL; //TBX_MALLOC(sizeof(mad_track_set_t));
  //initialize_tracks(mad_adapter);

  // -> Détermination du nombre de ports de réception
  // -> ouverture des ports
  // mise en place des structures d'accès aux zones à recevoir
  // -> mise en place des zones pré-postées si nécessaire
