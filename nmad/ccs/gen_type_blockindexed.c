...
int CCSI_datadesc_blockindexed_count_contig(int count,
					     int blklen,
					     CCS_aint_t *disp_array,
					     CCS_aint_t old_extent);

static void DLOOP_Type_blockindexed_array_copy(int count,
					       void *disp_array,
					       CCS_aint_t *out_disp_array,
					       int dispinbytes,
					       CCS_aint_t old_extent);
						 ...
						 int flags)
{
    ...
    if (is_builtin)
    {
	DLOOP_Handle_get_basic_size_macro(oldtype, old_extent);
	old_loop_depth = 0;
    }
    else
    {
	...
	(((CCS_aint_t) ((int *) disp_array)[0]) * old_extent);
