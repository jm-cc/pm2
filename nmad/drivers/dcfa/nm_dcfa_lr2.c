...
#define NM_DCFA_LR2_BUFSIZE (512*1024)
#define NM_DCFA_LR2_NBUF 3

#define NM_DCFA_LR2_BLOCKSIZE 4096
...
static inline nm_len_t nm_dcfa_min(const nm_len_t a, const nm_len_t b)
{
  if(b > a)
    return a;
  else
...
{  
  struct
  {
    char sbuf[NM_DCFA_LR2_BUFSIZE * NM_DCFA_LR2_NBUF];
    char rbuf[NM_DCFA_LR2_BUFSIZE * NM_DCFA_LR2_NBUF];
...
static int  nm_dcfa_lr2_poll_one(void*_status);

static const struct nm_minidriver_iface_s nm_dcfa_lr2_minidriver =
  {
    ...
    .poll_one    = &nm_dcfa_lr2_poll_one,
    .cancel_recv = NULL
  };

static void*nm_dcfa_lr2_instantiate(puk_instance_t instance, puk_context_t context);
static void nm_dcfa_lr2_destroy(void*);

static const struct puk_component_driver_s nm_dcfa_lr2_component =
  {
    .instantiate = &nm_dcfa_lr2_instantiate,
...
static int nm_dcfa_lr2_send_poll(void*_status)
{
  ...
  while(lr2->send.done < lr2->send.size)
    {
      ...
      const nm_len_t block_size = NM_DCFA_LR2_BLOCKSIZE;
      ...
      const nm_len_t chunk_payload = nm_dcfa_min(lr2->send.size - lr2->send.done, chunk_max_payload);
      /* ** N-buffering */
      if((lr2->send.sbuf + chunk_size) > (((void*)lr2->buffer.sbuf) + (lr2->send.nbuffer + 1) * NM_DCFA_LR2_BUFSIZE))
	{
	   ...
	   * */
	  const int n2 = (lr2->send.nbuffer + 2) % NM_DCFA_LR2_NBUF;
	  if(lr2->buffer.rack == n2)
	    { 
	      return -NM_EAGAIN;
	    }
	  /* swap buffers */
	  lr2->send.nbuffer = (lr2->send.nbuffer + 1) % NM_DCFA_LR2_NBUF;
	  lr2->send.sbuf = ((void*)lr2->buffer.sbuf) + lr2->send.nbuffer * NM_DCFA_LR2_BUFSIZE;
	  lr2->send.rbuf = ((void*)lr2->buffer.rbuf) + lr2->send.nbuffer * NM_DCFA_LR2_BUFSIZE;
	}
      ...
      else
	{
	  ...
	  const nm_len_t padding = (nm_dcfa_alignment > 0) ? (base_offset % nm_dcfa_alignment) : 0;
	  ...
	  while(chunk_todo > 0)
		...
		block_max_payload : (chunk_todo % block_max_payload);
	      ...
	      h->checksum = 1 | nm_dcfa_memcpy_and_checksum(lr2->send.sbuf + chunk_offset, lr2->send.message + lr2->send.done + (chunk_payload - chunk_todo), block_payload);
	      chunk_todo   -= block_payload;
	      chunk_offset += block_payload + lr2_hsize;
	    }
	}
      /* ** send chunk */
      nm_dcfa_send_flushn(lr2->cnx, NM_DCFA_WRID_PACKET, 1);
      nm_dcfa_rdma_send(lr2->cnx, chunk_offset - base_offset, lr2->send.sbuf + base_offset, lr2->send.rbuf + base_offset,
			   &lr2->buffer, &lr2->seg, lr2->mr, NM_DCFA_WRID_PACKET);
  ...
  nm_dcfa_send_flush(lr2->cnx, NM_DCFA_WRID_PACKET);
...
static void nm_dcfa_lr2_send_prefetch(void*_status, const void*ptr, uint64_t size)
{
  ...
  if((lr2->send.prefetch == NULL) && (lr2->send.message == NULL))
      ...
      while(chunk_offset < chunk_size)
	{
	  ...
	  h->checksum = 1 | nm_dcfa_memcpy_and_checksum(&lr2->buffer.sbuf[chunk_offset], ptr + chunk_done, block_payload);
  ...
  while(lr2->recv.done < lr2->recv.size)
    {
      ...
      const nm_len_t block_max_payload = block_size - lr2_hsize;
      const nm_len_t chunk_max_payload = chunk_size - lr2_hsize * chunk_size / block_size;
      const nm_len_t chunk_payload = nm_dcfa_min(lr2->recv.size - lr2->recv.done, chunk_max_payload);
      if((lr2->recv.rbuf + chunk_size) > (((void*)lr2->buffer.rbuf) + (lr2->recv.nbuffer + 1) * NM_DCFA_LR2_BUFSIZE))
      {
	/* swap buffers */
	lr2->recv.nbuffer = (lr2->recv.nbuffer + 1) % NM_DCFA_LR2_NBUF;
	lr2->recv.rbuf = ((void*)lr2->buffer.rbuf) + lr2->recv.nbuffer * NM_DCFA_LR2_BUFSIZE;
	nm_dcfa_send_flush(lr2->cnx, NM_DCFA_WRID_ACK);
	lr2->buffer.sack = lr2->recv.nbuffer;
	nm_dcfa_rdma_send(lr2->cnx, sizeof(uint32_t),
			     (void*)&lr2->buffer.sack, (void*)&lr2->buffer.rack,
			     &lr2->buffer, &lr2->seg, lr2->mr, NM_DCFA_WRID_ACK);
      }
      nm_len_t chunk_todo = chunk_payload;
      ...
      while(chunk_todo > 0)
	{
	  ...
	  const uint32_t checksum =
	    1 | nm_dcfa_memcpy_and_checksum(lr2->recv.message + lr2->recv.done, lr2->recv.rbuf + chunk_offset, block_payload);
