...
int ma_supervisor_read_command(int fd, struct ma_supervisor_msg *p_msg)
{
	...
	r = marcel_read(fd, p_msg, sizeof(*p_msg));
	_errno = errno;
...
void marcel_supervisor_init(void)
{
	...
	ma_supervisor_fd = marcel_open(filename, O_RDONLY | O_NONBLOCK);
	...
	if (reverse_filename) {
		fprintf(stderr, "marcel_supervisor_init: - reverse fifo filename = %s\n",
			reverse_filename);
		ma_supervisor_reverse_fd = marcel_open(reverse_filename, O_RDWR | O_NONBLOCK);
	...
	if (neighbour_filename) {
		...
		ma_supervisor_neighbour_fd =
			marcel_open(neighbour_filename, O_RDWR | O_NONBLOCK);
