...
void tbx_htable_init(p_tbx_htable_t htable, tbx_htable_bucket_count_t buckets)
{
	PM2_LOG_IN();
	if (!buckets)
...
{
	tbx_htable_element_count_t count;
...
static
tbx_htable_bucket_count_t __tbx_htable_get_bucket(p_tbx_htable_t htable,
						const char *key, size_t key_len)
{
	...
	PM2_LOG_IN();
	while (key_len--)
		bucket += (unsigned char)key[key_len];
...
void tbx_htable_add_ext(p_tbx_htable_t htable,
			const char *key, const size_t key_len, void *object)
{
	tbx_htable_bucket_count_t bucket = 0;
	p_tbx_htable_element_t element;
...
{
	tbx_htable_bucket_count_t bucket;
	p_tbx_htable_element_t element;
			 ...
			 const char *key, const size_t key_len)
{
	...
	result = (element) ? element->object : NULL;
			     ...
			     const char *key, const size_t key_len)
{
	...
	while (*element) {
		if ((*element)->key_len != key_len || memcmp(key, (*element)->key, key_len))
			...
			p_tbx_htable_element_t tmp_element;
			void *object;
	...
	if (htable->nb_element) {
		...
		while (bucket--) {
			...
			while (element) {
				...
				tbx_htable_keystr(element->key, element->key_len, keystr);
				PM2_DISP("%s: '%s'", keystr, (char *) (element->object));
				element = element->next;
			}
		}
	}
	...
	if (htable->nb_element) {
		...
		while (bucket--) {
			...
			while (element) {
				...
				tbx_htable_keystr(element->key, element->key_len, keystr);
				PM2_DISP("%s: %p", keystr, element->object);
				element = element->next;
			}
		}
	}
