...
int check_buffer(char*buffer, int len, int offset)
{
  ...
  for( i=0; i<len; i++) {
    if(buffer[i] != 'a' + ((i + offset)%26)) {
      printf("Corrupted data at byte #%d (offset %d): car is %c (%d), expected: %c (%d)\n",
...
#ifdef DEBUG
    fprintf(stderr, "### Irecv %d * %d bytes ###\n", NB_IOV, SIZE);
#endif
...
#ifdef DEBUG
    fprintf(stderr, "### Isend %d * %d bytes ###\n", NB_IOV, SIZE);
#endif
...
#endif
    if(nm_sr_isend(p_session, gate_id, 42, 
		   sbuf, NB_IOV * SIZE, 
		   &sreq) != NM_ESUCCESS) {
      fprintf(stderr, "NM Isend failed \n");
      abort();
...
#endif
    if(nm_sr_irecv(p_session, gate_id, 42, 
		   rbuf, NB_IOV * SIZE, 
		   &rreq) != NM_ESUCCESS) {
      fprintf(stderr, "NM Irecv failed \n");
      abort();
