...
#define __MA_ARCH_IRQ_STAT	1

//#define ma_softirq_pending(cpu)		(ma_cpu_data(cpu)->softirq_pending)
#define ma_syscall_count(cpu)		/* unused on IA-64 */
#define ma_ksoftirqd_task(cpu)		(ma_cpu_data(cpu)->ksoftirqd)
#define ma_nmi_count(cpu)			0

//#define ma_local_softirq_pending()		(ma_local_cpu_data->softirq_pending)
#define ma_local_syscall_count()		/* unused on IA-64 */
#define ma_local_ksoftirqd_task()		(ma_local_cpu_data->ksoftirqd)
#define ma_local_nmi_count()		0
...
//# define ma_in_atomic()		((ma_preempt_count() & ~MA_PREEMPT_ACTIVE) != kernel_locked())
# define ma_in_atomic()		((ma_preempt_count() & ~MA_PREEMPT_ACTIVE))

# define MA_IRQ_EXIT_OFFSET (MA_HARDIRQ_OFFSET-1)


// d'apres asm-i386/linux-hardirq.h
...
do {									\
		...
		if (!ma_in_interrupt() && ma_softirq_pending(GET_LWP(MARCEL_SELF))) \
			ma_do_softirq();					\
		/*ma_preempt_enable_no_resched();*/				\
		ma_preempt_enable();				\
} while (0)

// fin d'emprunt a i386
