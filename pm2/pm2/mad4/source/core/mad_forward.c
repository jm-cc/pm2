			       ...
			       p_mad_fblock_header_t fbh)
{
  ...
  mad_send_mode_t          send_mode  = mad_send_SAFER;
  mad_receive_mode_t       recv_mode  = mad_receive_EXPRESS; 
...
mad_forward_register(p_mad_driver_t driver)
{
  ...
  interface->driver_init                = mad_forward_driver_init;
  interface->adapter_init               = NULL;
  interface->channel_init               = mad_forward_channel_init;
  interface->before_open_channel        = mad_forward_before_open_channel;
  interface->connection_init            = mad_forward_connection_init;
  interface->link_init                  = mad_forward_link_init;
  interface->accept                     = NULL;
  interface->connect                    = NULL;
  interface->after_open_channel         = mad_forward_after_open_channel;
  interface->before_close_channel       = mad_forward_before_close_channel;
  interface->disconnect                 = mad_forward_disconnect;
  interface->after_close_channel        = mad_forward_after_close_channel;
  interface->link_exit                  = mad_forward_link_exit;
  interface->connection_exit            = mad_forward_connection_exit;
  interface->channel_exit               = mad_forward_channel_exit;
  interface->adapter_exit               = NULL;
  interface->driver_exit                = NULL;
  interface->choice                     = mad_forward_choice;
  interface->get_static_buffer          = mad_forward_get_static_buffer;
  interface->return_static_buffer       = mad_forward_return_static_buffer;
  interface->new_message                = mad_forward_new_message;
  interface->finalize_message           = mad_forward_finalize_message;
  interface->message_received           = mad_forward_message_received;
#ifdef MAD_MESSAGE_POLLING
  interface->poll_message               = mad_forward_poll_message;
#endif // MAD_MESSAGE_POLLING
  interface->receive_message            = mad_forward_receive_message;
  interface->send_buffer                = mad_forward_send_buffer;
  interface->receive_buffer             = mad_forward_receive_buffer;
  interface->send_buffer_group          = mad_forward_send_buffer_group;
  interface->receive_sub_buffer_group   = mad_forward_receive_sub_buffer_group;
  LOG_OUT();
}
  ...
  if (connection->nature == mad_connection_nature_direct_virtual)
    {
      ...
      interface = regular->channel->adapter->driver->interface;
       ...
       regular_lnk = interface->choice(regular, size,
				       send_mode, smode_size,
				       receive_mode, rmode_size);
       lnk = connection->link_array[regular_lnk->id]; // GM ; pb here !!!
    }
  else if (connection->nature ==
	   mad_connection_nature_indirect_virtual)
    {
