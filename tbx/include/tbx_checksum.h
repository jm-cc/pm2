 ...
 */
static inline uint32_t tbx_checksum_dummy(const void*_data TBX_UNUSED, size_t _len TBX_UNUSED)
{
	return 0;
}
...
static inline uint32_t tbx_checksum_jenkins(const void*_data, size_t _len)
{
	...
	return (uint32_t)((hash & 0xFFFFFFFF) ^ ((hash & 0xFFFFFFFF00000000ULL) >> 32));
}
 ...
 */
static inline uint32_t tbx_checksum_murmurhash64a(const void*_data, size_t len)
{
