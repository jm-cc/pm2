...
#define marcel_print_level_description_level(_l) \
  if (type & (1<<_l)) { \
    fprintf(output, "%s%s", current_separator, marcel_topo_level_string(_l)); \
...
#define marcel_physindex_printf(_prefix, _index) \
  ({ char *__tmp = alloca(MA_PHYSINDEX_STRLEN); __tmp[0] = '\0'; if (_index != -1) snprintf(__tmp, MA_PHYSINDEX_STRLEN, "%s%d", _prefix, _index); __tmp; })
			...
			const char *labelseparator, const char *levelterm)
{
	...
	if (l->merged_type & (1 << MARCEL_LEVEL_NODE))
		fprintf(output, "%sNode%s(%lu%s)", separator,
	...
	if (l->merged_type & (1 << MARCEL_LEVEL_L2))
		fprintf(output, "%sL2%s(%lu%s)", separator,
	...
	if (l->merged_type & (1 << MARCEL_LEVEL_PROC))
		fprintf(output, "%sCPU%s", separator,
...
{
	marcel_vps_per_cpu =
	    (marcel_nbvps() + marcel_nbprocessors - 1) / marcel_nbprocessors;
		...
		for (i = 0; !marcel_vpset_iszero(&marcel_topo_levels[l][i].cpuset); i++) {
			if (marcel_topo_levels[l][i].arity) {
				MARCEL_LOG("level %u,%u: cpuset %" MARCEL_PRIxVPSET
					   " arity %u\n", l, i,
					   MARCEL_VPSET_PRINTF_VALUE(marcel_topo_levels[l]
								     [i].cpuset),
					   marcel_topo_levels[l][i].arity);
				marcel_topo_levels[l][i].children =
				    TBX_MALLOC(marcel_topo_levels[l][i].arity *
					       sizeof(void *));
				MA_ALWAYS_BUG_ON(!marcel_topo_levels[l][i].children);

				m = 0;
				for (j = 0;
				     !marcel_vpset_iszero(&marcel_topo_levels[l + 1]
							  [j].cpuset); j++)
...
static enum marcel_topo_level_e ma_topo_level_type_from_hwloc(hwloc_obj_t t)
{
	...
	case HWLOC_OBJ_SYSTEM:
		...
		break;
	case HWLOC_OBJ_MACHINE:
		fprintf(stderr, "SSI OSes like Kerrighed not supported yet\n");
		MA_ALWAYS_BUG_ON(1);
#endif
	...
	case HWLOC_OBJ_CACHE:{
			...
			default:
				...
				fprintf(stderr, "Cannot convert hwloc cache depth %u\n",
					t->attr->cache.depth);
				MA_ALWAYS_BUG_ON(1);
			}
			break;
	...
	default:
		...
		fprintf(stderr, "Cannot convert hwloc type %u\n", t->type);
		MA_ALWAYS_BUG_ON(1);
	}
...
#  ifdef MA__NUMA
	...
	for (l = topodepth - 1; l != (unsigned) -1 && l < topodepth; l--) {
		...
		if (ltype ==
			...
			mlevels =
			    malloc((nbitems + MARCEL_NBMAXVPSUP + 1) * sizeof(*mlevels));
		...
		for (i = 0; i < nbitems; i++) {
			...
			mlevel->index =
			    hwloc_get_obj_by_depth(topology, l, i)->sibling_rank;
...
#else
				mlevel->memory_kB[MARCEL_TOPO_LEVEL_MEMORY_NODE] =
				    tlevel->attr->node.memory_kB;
				mlevel->huge_page_free =
				    tlevel->attr->node.huge_page_free;
#endif
				break;
			case MARCEL_LEVEL_DIE:
	...
	if (marcel_nb_vp == 0) {
		if (marcel_cpu_stride)
			...
			marcel_nb_vp = marcel_nbprocessors;
	}
	...
	struct marcel_topo_level *vp_level =
		malloc((marcel_nbvps() + MARCEL_NBMAXVPSUP + 1) * sizeof(*vp_level));
	...
	if (marcel_topo_max_arity) {
		...
		for (l = 0; l + 1 < marcel_topo_nblevels; l++) {
			...
			if (dosplit) {
				...
				marcel_topo_levels[l + 1] =
				    malloc((level_width + MARCEL_NBMAXVPSUP +
					    1) * sizeof(**marcel_topo_levels));
				...
				for (i = 0, j = 0;
					...
					marcel_topo_levels[l][i].children =
					    TBX_MALLOC(nbsublevels * sizeof(void *));
					for (k = 0; k < nbsublevels; k++)
...
#    ifndef MARCEL_SMT_IDLE
	for (l = 0; l + 1 < marcel_topo_nblevels; l++)
		for (i = 0; !marcel_vpset_iszero(&marcel_topo_levels[l][i].cpuset); i++)
			if (marcel_topo_levels[l][i].merged_type &=
			    1 << MARCEL_LEVEL_CORE) {
				fprintf(stderr,
					"Warning: several VPs are used on the same core, but the MARCEL_SMT_IDLE option is not set to optimize core sleep!\n");
				goto out;
			}
      out:
	(void) 0;
