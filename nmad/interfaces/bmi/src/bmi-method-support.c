...
method_op_p bmi_alloc_method_op(bmi_size_t payload_size)
{
    ...
    /* generic component */
    my_method_op = (method_op_p) TBX_MALLOC(ssize + payload_size);
    if (!my_method_op)
    {
	return (NULL);
    }
					      ...
					      bmi_size_t payload_size)
{
    ...
    /* generic component */
    my_method_addr = (struct bmi_method_addr *) TBX_MALLOC(ssize + payload_size);
    if (!my_method_addr)
    {
	return (NULL);
    }
		 ...
		 const char *id_string)
{
    ...
    if ((newkey = (char *) TBX_MALLOC(keysize + 4)) == NULL)
    {
	return (NULL);
    }
    strcpy(newkey, key);
    ...
    retsize = (end - holder);
    if ((retstring = (char *) TBX_MALLOC(retsize + 1)) == NULL)
    {
	TBX_FREE(newkey);
	return (NULL);
    }
