	...
	for (i = 0; i < 10000; ++i) {
		...
		if (r != PTHREAD_CREATE_DETACHED && r != PTHREAD_CREATE_JOINABLE) {
			...
			if (s != PTHREAD_CREATE_JOINABLE) {
				printf("detach state changed to %d by invalid setdetachstate call\n", s);
				exit(1);
			}
		}
		...
		if (r != PTHREAD_INHERIT_SCHED && r != PTHREAD_EXPLICIT_SCHED) {
			...
			if (s != PTHREAD_INHERIT_SCHED) {
				printf("inheritsched changed to %d by invalid setinheritsched call\n", s);
				exit(1);
			}
		}
		...
		if (r != SCHED_OTHER && r != SCHED_RR && r != SCHED_FIFO) {
			...
			if (s != SCHED_OTHER) {
				printf("schedpolicy changed to %d by invalid setschedpolicy call\n", s);
				exit(1);
			}
		}
		...
		if (r != PTHREAD_SCOPE_SYSTEM && r != PTHREAD_SCOPE_PROCESS) {
			...
			if (s != PTHREAD_SCOPE_SYSTEM) {
				printf("contentionscope changed to %d by invalid setscope call\n", s);
				exit(1);
			}
		}
		...
		if (r != PTHREAD_PROCESS_PRIVATE && r != PTHREAD_PROCESS_SHARED) {
			...
			if (s != PTHREAD_PROCESS_PRIVATE) {
				printf("pshared changed to %d by invalid mutexattr_setpshared call\n", s);
				exit(1);
			}
			...
			if (s != PTHREAD_PROCESS_PRIVATE) {
				printf("pshared changed to %d by invalid rwlockattr_setpshared call\n", s);
				exit(1);
			}
		}
