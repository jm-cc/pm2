...
struct GlobalMemory {
  int id;
  marcel_mutex_t idlock;
  marcel_bar_t_np start;
  unsigned long *transtimes;
  unsigned long *totaltimes;
...
unsigned long transtime2 = 0;
unsigned long avgtranstime = 0;
unsigned long avgcomptime = 0;
unsigned unsigned long transstart = 0;
unsigned unsigned long transend = 0;
unsigned long maxtotal=0;
unsigned long mintotal=0;
double maxfrac=0;
double minfrac=0;
...
void FFT1D(int,int,int,double *,double *,double *,double *,int,unsigned long *,int,
...

{
  ...
  GET_MILISECONDS(start);

  marcel_init(&argc, argv); timing_init();marcel_mutex_init(&marcel_end_lock_np, NULL);marcel_cond_init(&marcel_end_cond_np, NULL);;
  ...
  Global->transtimes = (unsigned long *) malloc(P*sizeof(unsigned long));  
  Global->totaltimes = (unsigned long *) malloc(P*sizeof(unsigned long));  
  if (Global == NULL) {
...
/* POSSIBLE ENHANCEMENT:  Here is where one might distribute the x,
  ...
  marcel_barinit_np(&Global->start);
  marcel_mutex_init(&Global->idlock, NULL);
  Global->id = 0;
...
#ifdef DEBUG
  printf("starting thread creation, I am %p \n",marcel_self());
#endif
  /* fire off P processes */
  for (i=1; i<P; i++) {
    marcel_create_np(SlaveStart);
  }
  SlaveStart();

  marcel_wait_for_end_np(P-1);
#ifdef DEBUG
  printf("end detected, I am %p, #threads = %d\n",marcel_self(), marcel_end_np_np);
#endif
  ...
  printf("Total time with initialization    : %16u\n",
	  (Global->finishtime-Global->starttime)/SCALE);
  printf("Total time without initialization : %16u\n",
	  (Global->finishtime-Global->initdonetime)/SCALE);
...
#ifdef DEBUG2
  printf("before main end, I am %p\n",marcel_self());
#endif
  marcel_end_np();
#ifdef DEBUG2
  printf("after main end, I am %p\n",marcel_self());
#endif
...
void SlaveStart()
{
  ...
  unsigned long l_transtime=0;
  int MyFirst; 
  int MyLast;

  marcel_mutex_lock(&Global->idlock);
    MyNum = Global->id;
    Global->id++;
  marcel_mutex_unlock(&Global->idlock); 
#ifdef DEBUGX
  printf("slave start, I am %p, MyNum = %d\n",marcel_self(),MyNum);
#endif

/* POSSIBLE ENHANCEMENT:  Here is where one might pin processes to
   processors to avoid migration */

  marcel_barrier_np(&Global->start, P);

  upriv = (double *) tmalloc(2*(rootN-1)*sizeof(double));  
  if (upriv == NULL) {
  ...
  marcel_barrier_np(&Global->start, P);
#ifdef DEBUG
  printf("after barrier 2, I am %p, MyNum = %d\n",marcel_self(),MyNum);
#endif
...
#ifdef DEBUG
  printf("after FFT, I am %p, MyNum = %d\n",marcel_self(),MyNum);
#endif
  if ((MyNum == 0) || (dostats)) {
    GET_MILISECONDS(finish);
...
#ifdef DEBUG
  printf("slave ended, before signal, I am %p, MyNum = %d\n",marcel_self(),MyNum);
#endif
  marcel_signal_end_np(MyNum, P-1);
#ifdef DEBUG
  printf("slave ended, I am %p, MyNum = %d\n",marcel_self(),MyNum);
#endif
}
