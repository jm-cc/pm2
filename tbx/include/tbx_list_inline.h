...
static
__inline__ void tbx_list_init(p_tbx_list_t list)
{
	list->length = 0;
...
static __inline__ void tbx_destroy_list(p_tbx_list_t list)
{
	if (list->length) {
		...
		while (list_element != list->last) {
			p_tbx_list_element_t temp_list_element =
				list_element->next;
...
__inline__
void
tbx_foreach_destroy_list(p_tbx_list_t list, p_tbx_list_foreach_func_t func)
{
	if (list->length) {
		p_tbx_list_element_t list_element;
 ...
 */
static
__inline__ void tbx_append_list(p_tbx_list_t list, void *object)
{
	p_tbx_list_element_t list_element;
	...
	list_element =
		(p_tbx_list_element_t) tbx_malloc(tbx_list_manager_memory);
...
static __inline__ void *tbx_get_list_object(p_tbx_list_t list)
{
	if (list->length) {
...
static __inline__ tbx_bool_t tbx_empty_list(p_tbx_list_t list)
{
	if (list->length) {
		return tbx_false;
	} else {
...
static
__inline__
void tbx_duplicate_list(p_tbx_list_t source, p_tbx_list_t destination)
{
	*destination = *source;
...
__inline__
void
tbx_extract_sub_list(p_tbx_list_t source, p_tbx_list_t destination)
{
	if (source->length) {
		destination->first = source->mark;
		destination->last = source->last;
		destination->length =
			source->length - source->mark_position;
		destination->mark = destination->first;
		destination->mark_position = 0;
		destination->read_only = tbx_true;
	} else
...
__inline__
void
tbx_list_reference_init(p_tbx_list_reference_t ref, p_tbx_list_t list)
{
	ref->list = list;
...
static
__inline__ void *tbx_get_list_reference_object(p_tbx_list_reference_t ref)
{
	if (!ref->reference) {
		if (ref->list->length) {
			ref->reference = ref->list->first;
...
static __inline__ void tbx_reset_list_reference(p_tbx_list_reference_t ref)
{
	if (ref->list->length) {
		ref->reference = ref->list->first;
		ref->after_end = tbx_false;
