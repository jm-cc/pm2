...
#ifndef MA_NR_VPS
#error MA_NR_VPS undefined!
#endif
#if !MA_NR_VPS
#error MA_NR_VPS is zero!
#endif
...
#    define MARCEL_VPSET_PRINTF_VALUE(x)	({			\
	...
	for(__i=MA_VPSUBSET_COUNT-1; __i>=0; __i--)			\
	  __tmp += marcel_sprintf(__tmp, MA_PRIxVPSUBSET ",", (x).s[__i]);	\
	*(__tmp-1) = '\0';						\
	__buf;								\
     })
...
#define PMARCEL_CPU_SETSIZE (sizeof(marcel_vpset_t)*8)
...
#else
#  define marcel_topo_core_level marcel_topo_vp_level
...
static __tbx_inline__ void marcel_vpset_vp(marcel_vpset_t * set, unsigned vp);
/** \brief Clear VP set and set all but the VP \e vp */
static __tbx_inline__ void marcel_vpset_all_but_vp(marcel_vpset_t * set, unsigned vp);
/** \brief Add VP \e vp in VP set \e set */
static __tbx_inline__ void marcel_vpset_set(marcel_vpset_t * set, unsigned vp);
/** \brief Remove VP \e vp from VP set \e set */
static __tbx_inline__ void marcel_vpset_clr(marcel_vpset_t * set, unsigned vp);
/** \brief Test whether VP \e vp is part of set \e set */
static __tbx_inline__ int marcel_vpset_isset(const marcel_vpset_t * set, unsigned vp);
...
marcel_topo_level_t *marcel_topo_common_ancestor(marcel_topo_level_t * lvl1, marcel_topo_level_t * lvl2);
...
#define marcel_malloc_node(size, node)     ma_malloc_node(size, node)
#define marcel_free_node(ptr, size, node)  ma_free_node(ptr, size)
...
#define marcel_vpset_foreach_begin(vp, vpset)	\
	vp = marcel_vpset_first(vpset);		\
	while (tbx_likely(-1 != vp)) {
#define marcel_vpset_foreach_end(vp, vpset)	\
	vp = marcel_vpset_next(vpset, vp);	\
	}

#define ma_current_os_node() (marcel_current_node_level() ? marcel_current_node_level()->os_node : 0)
...
struct marcel_topo_level {
	...
	void *ltask_data;		/**< \brief Task list for pioman / not use for marcel_io manager */
	...
	unsigned blocked;               /** < \brief Number of blocked LWP (in blocking section + spare) */
	unsigned spare;                 /** < \brief Number of spare LWP */
	int needed;                     /** < \brief Number of an empty VP which can home a spare/blocked LWP */
#endif
...
/** \brief Machine level */
extern struct marcel_topo_level marcel_machine_level[1 + MARCEL_NBMAXVPSUP + 1];
#ifdef MA__NUMA
...
extern char *ma_synthetic_topology_description;
...
/** \brief Compute the next VP in VP mask */
...
#define marcel_current_die_level() marcel_vp_die_level(ma_vpnum(MA_LWP_SELF))
...
#define marcel_current_core_level() marcel_vp_core_level(ma_vpnum(MA_LWP_SELF))
...
#define marcel_current_node_level() marcel_vp_node_level(ma_vpnum(MA_LWP_SELF))
...
/** \brief Returns true if _level_ is inside the subtree beginning
    with _subtree_root_. */
int ma_topo_is_in_subtree(marcel_topo_level_t * subtree_root,
			  marcel_topo_level_t * level);
