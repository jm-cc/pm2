...
#ifdef HEADER
static const int param_header_size       = 32;
static const int param_header_mode       = mad_receive_EXPRESS;
#endif
  ...
  if (process_lrank == master_lrank)
      ...
      if (!results)
	{
	  ...
	  MAD_end_unpacking(in);
	     ...
	     DISP("An error has occured in unpacking sequence");
  ...
  else if (results)
    {
	 ...
	 DISP("An error has occured in packing sequence");
...
#ifdef HEADER
      char              in_buff[param_header_size];  
      char              out_buff[param_header_size];  
#endif       
      sum = 0;
	  ...
	  while (nb_tests--)
	    {
	      ...
	      connection = MAD_begin_packing(channel, lrank_dst);
#ifdef HEADER	       
	      MAD_pack(connection, out_buff, param_header_size,
		       param_send_mode, param_header_mode);
#endif
      ...
      else
	{
	  ...
	  while (nb_tests--)
	      ...
	      while (nb_samples--)
		{
		  ...
		  connection = MAD_begin_unpacking(channel);
#ifdef HEADER	       
		  MAD_unpack(connection, in_buff, param_header_size,
			     param_send_mode, param_header_mode);
#endif 
