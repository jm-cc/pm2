...
#if defined(MARCEL_MAMI_ENABLED) && defined(MARCEL_BUBBLES) && defined(WHITEBOX)
...
main (int argc, char *argv[]) 
{
	...
	((long *) marcel_task_stats_get (marcel_self (), MEMNODE))[0] = 0;
	((long *) marcel_task_stats_get (marcel_self (), MEMNODE))[1] = 1024;
	...
	for (team = 0; team < NB_BUBBLES; team++) 
	{
		...
		for (i = team * THREADS_PER_BUBBLE; i < (team + 1) * THREADS_PER_BUBBLE; i++) 
		{
			...
			((long *) marcel_task_stats_get (threads[i], MEMNODE))[0] = (team == 0) ? 0 : 1024;
			((long *) marcel_task_stats_get (threads[i], MEMNODE))[1] = (team == 0) ? 1024 : 0;
		}
	}
