...
void pack_datatype_struct(nm_core_t             p_core,
  ...
  nm_pack(&cnx, &datatype->nb_elements, sizeof(int));
  ...
  nm_begin_packing(p_core, gate_id, 0, &cnx);
  nm_pack(&cnx, datatype->indices, numberOfBlocks*sizeof(int));
  ...
  for(j=0 ; j<datatype->nb_elements ; j++) {
    for(i=0 ; i<numberOfBlocks ; i++) {
      ...
      nm_pack(&cnx, tmp_buf[pack], datatype->blocklens[i]*size);
      pack++;
      if (pack % 256 == 0) {
...
void unpack_datatype_struct(nm_core_t            p_core,
  ...
  nm_unpack(&cnx, &count, sizeof(int));
  ...
  nm_begin_unpacking(p_core, gate_id, 0, &cnx);
  nm_unpack(&cnx, block_offset, numberOfBlocks*sizeof(int));
  nm_unpack(&cnx, block_elements, numberOfBlocks*sizeof(int));
  nm_end_unpacking(&cnx);

  nm_begin_unpacking(p_core, gate_id, 0, &cnx);
  for(i=0 ; i<numberOfBlocks ; i++) {
    nm_unpack(&cnx, &block_size[i], sizeof(int));
  }
  nm_end_unpacking(&cnx);
  ...
  for(j=0 ; j<count ; j++) {
    for(i=0 ; i<numberOfBlocks ; i++) {
      ...
      nm_unpack(&cnx, tmp_buf[unpack], block_elements[i]*block_size[i]);
      unpack++;
      if (unpack % 256 == 0) {
