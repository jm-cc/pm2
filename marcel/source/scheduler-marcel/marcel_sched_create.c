					   ...
					   __const unsigned long base_stack)
{
	...
	new_task->father->child = (dont_schedule ? NULL :	/* On ne fait rien */
	...
	marcel_ctx_set_new_stack(new_task, new_task->initial_sp, base_stack);
...
void marcel_sched_internal_create_dontstart_son(void)
{
	...
	MTRACE("On new stack", ma_self());

	//PROF_OUT_EXT(newborn_thread);
	PROF_SET_THREAD_NAME(MARCEL_SELF);

	if (MA_THR_SETJMP(ma_self()) == FIRST_RETURN) {
