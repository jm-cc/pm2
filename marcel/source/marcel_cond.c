...
{
	MARCEL_LOG_IN();
	cond->__data.__lock = (struct _marcel_fastlock) MA_MARCEL_FASTLOCK_UNLOCKED;
...
int marcel_cond_wait(marcel_cond_t * __restrict cond, marcel_mutex_t * __restrict mutex)
{
	MARCEL_LOG_IN();
	...
	ma_fastlock_release(&mutex->__data.__lock);
	{
		...
		__marcel_register_spinlocked(&cond->__data.__lock, ma_self(), &c);
			  ...
			  const struct timespec *__restrict abstime)
{
	...
	ma_fastlock_release(&mutex->__data.__lock);
	{
		...
		MARCEL_LOG("blocking %p (cell %p) in marcel_cond_timedwait %p\n",
			   ma_self(), &c, cond);
		...
		if (c.blocked) {
			if (__marcel_unregister_spinlocked(&cond->__data.__lock, &c)) {
				PM2_LOG
		...
		ma_fastlock_release(&cond->__data.__lock);
		MARCEL_LOG("unblocking %p (cell %p) in marcel_cond_timedwait %p\n",
			   ma_self(), &c, cond);
	}
		      ...
		      pmarcel_mutex_t * __restrict mutex)
{
	...
	ma_fastlock_release(&mutex->__data.__lock);
	{
		...
		__pmarcel_register_spinlocked(&cond->__data.__lock, ma_self(), &c);
...
#ifdef MARCEL_DEVIATION_ENABLED
	if (t->canceled == MARCEL_IS_CANCELED)
			   ...
			   const struct timespec *__restrict abstime)
{
	...
	ma_fastlock_release(&mutex->__data.__lock);
	{
		...
		if (c.blocked
			...
			if (__pmarcel_unregister_spinlocked(&cond->__data.__lock, &c)) {
				PM2_LOG
				    ("Strange, we should be in the queue !!! (%s:%d)\n",
				     __FILE__, __LINE__);
			}
