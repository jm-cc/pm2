...
\date{\footnotesize%
\url{$Id: start.tex,v 1.1 2001/04/02 09:42:26 bouge Exp $}%
}
...
\section{Installating PM2}

The following sections illustrate how to install PM2 on your machine.
...
\begin{shell}
tar xvfz pm2.tar.gz                # Using the GNU tar utility
\end{shell}
...
First, the \|PM2_ROOT| variable must contain the path to the PM2
distribution root directory. Second, the
\|${PM2_ROOT}/bin|
directory has to be included in the active search path.

Also, it is advisable to set the \|PM2_FLAVOR| to some default value.
You are advised to use \|pm2| in a first place: this instructs PM2 to
use the default TCP interface available on your machine. The precise
meaning of the variable will be explained below.
...
setenv PM2_FLAVOR   pm2
\end{shell}
  
\subsection{Creating PM2 flavors}
...
\begin{shell}
source ${HOME}/.cshrc           # If you are running csh
cd ${PM2_ROOT}
make init
\end{shell} 
...
\begin{shell}
ls -la  ${HOME}/.pm2/flavors
\end{shell}

As your \|PM2_FLAVOR| is currently set to \|pm2|, the set of
compilation and run option will be taken from the
\|${HOME}/.pm2/flavors/pm2|
file. Just have a look at it to get a feeling of your current
flavor. You'll probably never dare do it again!
\begin{shell}
less ${HOME}/.pm2/flavors/${PM2_FLAVOR}
\end{shell}
...
\subsection{Configuring your own flavors}
...
needs for the very first run at least. You can check it by invoking
the \|pm2-config| command:
\begin{shell}
pm2-config --protocols
\end{shell}

\subsection{Compiling the library}

You are now ready for the big thing: compiling the whole library.  All
the files generated within the compilation process will be placed
by default in the \|${HOME}/build|
directory. Though this may be convenient in a first place, you may
wish to keep these files in a more dedicated place. Just define the
\|PM2_BUILD_DIR| shell variable to the desired value. For instance, I
personally use the following assignment:
\begin{shell}
setenv PM2_BUILD_DIR ${PM2_ROOT}/build
\end{shell}
If you add this setting in your initialization file, do not forget to
re-source it before going further.
...
The PM2 library should now fully  configured. You are ready to
compile the example programs of the distribution. Let's start with the
...
this detail at this point: PM2 cares about it for you!
...
\|pm2conf| command. My current machine is called \|ravel|, and twho
...
\figurelisting {Makefile} {A minimal Makefile for PM2
  programs} {prog:Makefile}
...
\begin{note}
  Luc to Olivier: What about \|environ|?
\end{note}
