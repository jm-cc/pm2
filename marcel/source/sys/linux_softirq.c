...
/*
 * enum type to tell create_ksoftirqd_task to create a special thread
 *
 */
typedef enum {
	KSOFTIRQD_FOR_LWP,
...
inline static unsigned long __ma_do_softirq(unsigned long(*pending_hardirq_func)(void))
{
	int max_restart;
...
asmlinkage void ma_do_softirq(void)
{
	...
	/** process pending softirq for lwp level **/
	pending = __ma_do_softirq(local_softirq_pending_hardirq);
	if (pending) {
		/* Too much work, let's stop for now and just let the
		 * softirq daemon handle them */
		ma_softirq_pending_lwp(lwp) |= pending;
		ma_smp_wmb();
		ma_wakeup_lwp_softirqd();
	}

	/** process pending softirq for vp level **/
	pending = __ma_do_softirq(vp_softirq_pending_hardirq);
	if (pending) {
		/* Too much work, let's stop for now and just let the
		 * softirq daemon handle them */
		ma_softirq_pending_vp(vp) |= pending;
		ma_smp_wmb();
...
void marcel_tasklet_set_vpset(struct marcel_tasklet_struct *t TBX_UNUSED, marcel_vpset_t vp_set TBX_UNUSED)
{
#ifdef MARCEL_REMOTE_TASKLETS	
	t->vp_set = vp_set;
#endif
...
static inline marcel_task_t *create_ksoftirqd_start(kirqd_cat_t fam, ma_lwp_t lwp)
{
	marcel_t t;
	marcel_attr_t attr;
	...
	MARCEL_LOG_IN();
        ...
        /* Start thread responsible for exit */
	marcel_attr_init(&attr);
	if (fam == KSOFTIRQD_FOR_LWP) {
		marcel_snprintf(name, MARCEL_MAXNAMESIZE, "ksoftirqd/lwp%p", lwp);
		marcel_attr_setschedrq(&attr, ma_lwp_rq(lwp));
	} else {
		marcel_snprintf(name, MARCEL_MAXNAMESIZE, "ksoftirqd/vp%d", ma_vpnum(lwp));
...
#ifdef PM2
	{
		...
		marcel_attr_setstackaddr(&attr, 
					 (void *)((unsigned long)(stack+THREAD_SLOT_SIZE) & ~(THREAD_SLOT_SIZE-1)));
	}
#endif
...
static void ksoftirqd_start(ma_lwp_t lwp)
{
	...
	p = create_ksoftirqd_start(KSOFTIRQD_FOR_VP, lwp);
	ma_topo_vpdata_l(ma_per_lwp(vp_level, lwp), ksoftirqd) = p;
	p = create_ksoftirqd_start(KSOFTIRQD_FOR_LWP, lwp);
