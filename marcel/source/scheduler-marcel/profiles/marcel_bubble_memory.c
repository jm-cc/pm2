...
struct marcel_bubble_memory_sched {
  ...
  /** \brief Function to call to migrate all the memory areas attached to a bubble */
	bubble_migrate_all_func_t bubble_migrate_all_func;

  /** \brief Function to call to migrate all the memory areas attached to a task */
...
static void ma_memory_print_affinities(marcel_bubble_t * bubble)
{
	...
	fprintf(stderr, "Printing memory affinity hints for bubble %p:\n", bubble);

	for_each_entity_scheduled_in_bubble_begin(e, bubble);

	fprintf(stderr, "Entity %p (%s): [ ", e,
		e->type == MA_BUBBLE_ENTITY ? "bubble" : e->name);
	long *nodes = ma_cumulative_stats_get(e, ma_stats_memnode_offset);
	for (i = 0; i < marcel_nbnodes; i++) {
		fprintf(stderr, "%8ld%s", nodes[i],
			(i == (marcel_nbnodes - 1)) ? " ]\n" : ", ");
	}
...
static void ma_memory_print_previous_location(marcel_bubble_t * bubble)
{
	...
	marcel_mutex_lock(&print_lock);
	fprintf(stderr,
		"Printing previous location of threads scheduling in bubble %p:\n",
		bubble);
				 ...
				 task_migrate_all_func_t task_migrate_all_func)
{
	...
	scheduler->bubble_migrate_all_func = bubble_migrate_all_func;
	scheduler->task_migrate_all_func = task_migrate_all_func;
}

static int memory_sched_start(marcel_bubble_sched_t * self)
{
#if MA_MEMORY_BSCHED_NEEDS_DEBUGGING_FUNCTIONS
...
*/
static marcel_topo_level_t *ma_memory_favorite_level(marcel_entity_t * e)
{
	...
	case MA_BUBBLE_ENTITY:
		favorite_level =
		    marcel_topo_common_ancestor(&marcel_topo_node_level[first_node],
...
static int ma_entities_are_the_same(marcel_entity_t * e[], int ne)
{
	int i;
	int type = e[0]->type;
...
static int ma_memory_mem_load_compar(const void *e1, const void *e2)
{
	...
	return (mem_stats1[best_node] <=
		mem_stats2[best_node]) - (mem_stats1[best_node] >= mem_stats2[best_node]);
}
...
static int ma_memory_burst_light_bubbles(marcel_entity_t ** e, int ne, int arity)
{
	int i, new_ne = ne;
...
static int
ma_memory_global_balance(ma_distribution_t * distribution, unsigned int arity,
			 unsigned int load_per_level)
{
	...
	for (i = 0; i < arity; i++)
		qsort(distribution[i].entities,
		      ...
		      sizeof(distribution[i].entities[0]), ma_memory_mem_load_compar);

	while ((distribution[least_loaded].total_load + 1 <
		distribution[most_loaded].total_load)
		...
		if (distribution[most_loaded].nb_entities > 1) {
			...
			least_loaded =
			    ma_distribution_least_loaded_index(distribution, arity);
			most_loaded =
			    ma_distribution_most_loaded_index(distribution, arity);
		} else {
			return 1;
		}
	}
...
static int
ma_memory_apply_memory_distribution(void *memory_manager,
				    bubble_migrate_all_func_t bubble_migrate_all_func,
				    task_migrate_all_func_t task_migrate_all_func,
				    ma_distribution_t * distribution, unsigned int arity)
{
	unsigned int i, j;

	if (!memory_manager) {
		MARCEL_SCHED_LOG
		    ("`memory' scheduler: lacking a memory manager, nothing done\n");
		return 1;
	}
	...
	for (i = 0; i < arity; i++) {
		for (j = 0; j < distribution[i].nb_entities; j++) {
			...
			case MA_BUBBLE_ENTITY:
				bubble_migrate_all_func(memory_manager,
							ma_bubble_entity(distribution
									 [i].entities[j]),
							i);
				break;

			case MA_THREAD_ENTITY:
				task_migrate_all_func(memory_manager,
						      ma_task_entity(distribution
								     [i].entities[j]), i);
				break;
...
ma_memory_distribute(marcel_topo_level_t * from,
		     marcel_entity_t * e[], int ne, ma_distribution_t * distribution)
{
	unsigned int i, child, arity = from->arity, entities_per_level =
	    marcel_vpset_weight(&from->vpset) / arity;
	ma_distribution_t load_balancing_entities;
	ma_distribution_init(&load_balancing_entities, (marcel_topo_level_t *) NULL, 0,
			     ne);
	...
	for (i = 0; i < ne; i++) {
		...
		if (favorite_level) {
			for (child = 0; child < from->arity; child++) {
				if (ma_topo_is_in_subtree
				    (from->children[child], favorite_level))
					ma_distribution_add_tail(e[i],
								 &distribution[child]);
			}
		} else {
	...
	ma_memory_spread_load_balancing_entities(distribution,
						 &load_balancing_entities,
						 arity,
						 entities_are_the_same, ne / arity);
			...
			struct marcel_topo_level *from)
{
	...
	tbx_bool_t node_level_reached =
	    (distribution[0].level->type == MARCEL_LEVEL_NODE) ? tbx_true : tbx_false;

	for (i = 0; i < ne; i++) {
		struct marcel_topo_level *favorite_location =
		    ma_memory_favorite_level(e[i]);
	...
	if (node_level_reached)
		...
		ma_memory_apply_memory_distribution(scheduler->memory_manager,
						    scheduler->bubble_migrate_all_func,
						    scheduler->task_migrate_all_func,
						    distribution, arity);
...
ma_memory_sched_submit(struct marcel_bubble_memory_sched *self,
		       marcel_bubble_t * bubble, struct marcel_topo_level *from)
{
	MARCEL_SCHED_LOG("marcel_root_bubble: %p \n", &marcel_root_bubble);
...
static int memory_sched_submit(marcel_bubble_sched_t * self, marcel_entity_t * e)
{
	MA_BUG_ON(e->type != MA_BUBBLE_ENTITY);
	struct marcel_topo_level *from = ma_get_parent_rq(e)->topolevel;
	MARCEL_SCHED_LOG("Memory: Submitting entity %p from topo_level %s\n", e,
			 from->rq.as_holder.name);
	return ma_memory_sched_submit((struct marcel_bubble_memory_sched *) self,
				      ma_bubble_entity(e), from);
}

static int memory_sched_shake(marcel_bubble_sched_t * self)
{
	int ret = 0, shake = 0;
	marcel_bubble_t *holding_bubble =
	    ma_bubble_holder(ma_entity_task(ma_self())->natural_holder);
	marcel_entity_t *e;
	...
	for_each_entity_held_in_bubble(e, holding_bubble) {
		...
		if (parent_level && favorite_location) {
			if (marcel_topo_common_ancestor(parent_level, favorite_location) !=
			    favorite_location) {
				shake = 1;
				break;
			}
				  ...
				  struct memory_goodness_hints *hints)
{
	...
	computed_score +=
	    (topo_max_depth -
	     marcel_topo_common_ancestor(hints->from,
				     current_level)->level) *
	    MA_MEMORY_SCHED_REMOTENESS_PENALTY;
	...
	computed_score +=
	    (&marcel_topo_vp_level[current_e_last_vp] ==
	     hints->from) ? MA_MEMORY_SCHED_LAST_VP_BONUS : 0;
...
static void say_hello(marcel_entity_t * e, int current_score)
{
	printf("Looking at entity %s%s (%p), score = %i\n",
	       (e->type == MA_BUBBLE_ENTITY) ? "bubble" : "thread ",
	       (e->type == MA_BUBBLE_ENTITY) ? "" : e->name, e, current_score);
}

static int goodness(ma_holder_t * hold, void *args)
{
...
static int memory_sched_steal(marcel_bubble_sched_t * scheduler, unsigned from_vp)
{
	int ret = 0;
	...
	ma_topo_level_browse(me, MARCEL_LEVEL_MACHINE, goodness, &hints);

	printf("[Work Stealing] Best entity = %p (score = %i)\n", hints.best_entity,
	       hints.best_score);
...
static int memory_sched_exit(marcel_bubble_sched_t * scheduler)
{
	marcel_bubble_memory_sched_t *self;
				...
				tbx_bool_t work_stealing)
{
	...
	cache_sched =
	    malloc(marcel_bubble_sched_instance_size(&marcel_bubble_cache_sched_class));
	if (cache_sched) {
		/* Create a `cache' scheduler that will be invoked when `memory' can't
		   do a better job, e.g., within NUMA nodes.  */
		err =
		    marcel_bubble_cache_sched_init(cache_sched, marcel_topo_level(0, 0),
						   tbx_false);
		if (err)
			free(cache_sched);
		else {
			scheduler->cache_scheduler =
			    (marcel_bubble_sched_t *) cache_sched;
...
static int warning_start(marcel_bubble_sched_t * self TBX_UNUSED)
{
	printf
	    ("WARNING: You're currently trying to use the memory bubble scheduler, but as the enable_mami option is not activated in the current flavor, the bubble scheduler won't do anything! Please activate enable_mami in your flavor.\n");
	return 0;
}
...
static int make_default_scheduler(marcel_bubble_sched_t * scheduler)
{
	...
	scheduler->submit = (ma_bubble_sched_submit)warning_start;
	scheduler->shake = warning_start;
	return 0;
}
