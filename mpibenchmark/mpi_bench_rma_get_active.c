...
static void mpi_bench_get_ac_server(void*buf, size_t len)
{
...
static void mpi_bench_get_ac_client(void*buf, size_t len)
{
  MPI_Win_post(grp_other, 0, win);
  MPI_Win_wait(win);
  MPI_Win_start(grp_peer, 0, win);
...
static void mpi_bench_get_ac_init(void*buf, size_t len, int count)
{
...
static void mpi_bench_get_ac_finalize(void)
{
...
const struct mpi_bench_s mpi_bench_rma_get_active =
  {
    .label    = "mpi_bench_rma_get_active",
    .name     = "MPI RMA Get Active",
    .server   = &mpi_bench_get_ac_server,
    .client   = &mpi_bench_get_ac_client,
    .init     = &mpi_bench_get_ac_init,
    .finalize = &mpi_bench_get_ac_finalize
  };
