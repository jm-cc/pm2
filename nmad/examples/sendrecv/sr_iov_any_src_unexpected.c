...
void large_contigu_vers_large_contigu(void){
  ...
  *dst = '\0';
  ...
  if (is_server) {
    ...
    /* server */
    nm_sr_irecv(p_session, NM_ANY_GATE, 1, recv_plop, strlen(plop)+1, &request0);

    int i = 0;
    while(i++ < 100000)
      nm_sr_rtest(p_session, &request0);

    nm_sr_irecv_iov(p_session, NM_ANY_GATE, 0, iov, 1, &request1);
...
void large_contigu_vers_large_disperse(void){
  ...
  if (is_server) {
    ...
    nm_sr_irecv_iov(p_session, NM_ANY_GATE, 0, iov, 3, &request1);
    nm_sr_rwait(p_session, &request1);
...
void large_disperse_vers_large_disperse_disymetrique(void){
  ...
  if (is_server) {
    ...
    nm_sr_irecv_iov(p_session, NM_ANY_GATE, 0, iov, 2, &request1);
    nm_sr_rwait(p_session, &request1);
