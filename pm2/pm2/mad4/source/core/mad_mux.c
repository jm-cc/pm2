...
mad_mux_register(p_mad_driver_t driver)
{
  ...
  interface->driver_init                = mad_mux_driver_init;
  interface->adapter_init               = NULL;
  interface->channel_init               = mad_mux_channel_init;
  interface->before_open_channel        = mad_mux_before_open_channel;
  interface->connection_init            = mad_mux_connection_init;
  interface->link_init                  = mad_mux_link_init;
  interface->accept                     = NULL;
  interface->connect                    = NULL;
  interface->after_open_channel         = mad_mux_after_open_channel;
  interface->before_close_channel       = mad_mux_before_close_channel;
  interface->disconnect                 = mad_mux_disconnect;
  interface->after_close_channel        = mad_mux_after_close_channel;
  interface->link_exit                  = mad_mux_link_exit;
  interface->connection_exit            = mad_mux_connection_exit;
  interface->channel_exit               = mad_mux_channel_exit;
  interface->adapter_exit               = NULL;
  interface->driver_exit                = NULL;
  interface->choice                     = NULL;
  interface->get_static_buffer          = mad_mux_get_static_buffer;
  interface->return_static_buffer       = mad_mux_return_static_buffer;
  interface->new_message                = mad_mux_new_message;
  interface->finalize_message           = mad_mux_finalize_message;
  interface->message_received           = mad_mux_message_received;
#ifdef MAD_MESSAGE_POLLING
  interface->poll_message               = mad_mux_poll_message;
#endif // MAD_MESSAGE_POLLING
  interface->receive_message            = mad_mux_receive_message;
  interface->send_buffer                = mad_mux_send_buffer;
  interface->receive_buffer             = mad_mux_receive_buffer;
  interface->send_buffer_group          = mad_mux_send_buffer_group;
  interface->receive_sub_buffer_group   = mad_mux_receive_sub_buffer_group;
  interface->get_sub_channel            = mad_mux_get_sub_channel;
  LOG_OUT();
}
