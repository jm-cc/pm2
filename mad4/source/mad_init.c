             ...
             p_tbx_htable_t dir_adapter_htable)
{
  ...
  if(interface->sending_track_init){
    interface->sending_track_init(mad_adapter);
  } else {
    //p_mad_track_t    track = NULL;
    //p_mad_pipeline_t pipeline = NULL;
    //p_mad_on_going_t send = NULL;

    //send = TBX_MALLOC(sizeof(mad_on_going_t));
    //send->status = 0;
    //send->area_id = -1;
    //send->mad_iovec = NULL;
    //send->specific = NULL;
    //
    //grp = TBX_MALLOC(sizeof(mad_on_going_group_t));
    //grp->tab = send;
    //grp->nb_elm = 1;
    //grp->index_last = -1;
    //
    //set = TBX_MALLOC(sizeof(mad_on_going_set_t));
    //set->set = grp;
  }

  if(interface->reception_track_init){
    interface->reception_track_init(mad_adapter);
  } else {
    //p_mad_on_going_set_t set = NULL;
    //p_mad_on_going_group_t grp = NULL;
    //p_mad_on_going_t  reception= NULL;
    //
    //reception = TBX_MALLOC(sizeof(mad_on_going_t));
    //reception->status = 0;
    //reception->area_id = -1;
    //reception->mad_iovec = NULL;
    //reception->specific = NULL;
    //
    //grp = TBX_MALLOC(sizeof(mad_on_going_group_t));
    //grp->tab = reception;
    //grp->nb_elm = 1;
  ...
  mad_adapter->waiting_list = tbx_slist_nil();
  mad_adapter->large_mad_iovec_list = tbx_slist_nil();
  mad_adapter->sending_list = tbx_slist_nil();
  mad_adapter->receiving_list = tbx_slist_nil();
  /***/
...
mad_dir_channel_init(p_mad_madeleine_t madeleine)
{
...

  {
    ...
    nb_channels = tbx_htable_get_size(channel_htable);
    channel_tab = TBX_MALLOC(nb_channels * sizeof(p_mad_channel_t));

    key_list = tbx_htable_get_key_slist(channel_htable);
    tbx_slist_ref_to_head(key_list);
    do{
      key = tbx_slist_ref_get(key_list);

      channel = tbx_htable_get(channel_htable, key);

      if(channel == NULL)
        break;

      channel_tab[channel->id] = channel;
