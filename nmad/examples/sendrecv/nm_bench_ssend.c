...
static void sr_bench_ssend_server(void*buf, nm_len_t len)
{
  ...
  nm_sr_issend(nm_bench_common.p_session, nm_bench_common.p_gate, data_tag, buf, len, &request);
  nm_sr_swait(nm_bench_common.p_session, &request);

}

static void sr_bench_ssend_client(void*buf, nm_len_t len)
{
  nm_sr_request_t request;
  nm_sr_issend(nm_bench_common.p_session, nm_bench_common.p_gate, data_tag, buf, len, &request);
  nm_sr_swait(nm_bench_common.p_session, &request);
...
const struct nm_bench_s nm_bench =
  {
    .name = "sendrecv ssend",
    .server = &sr_bench_ssend_server,
    .client = &sr_bench_ssend_client
  };
