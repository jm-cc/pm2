...
static volatile unsigned __active_threads,
    __sleeping_threads, __blocked_threads, __frozen_threads;
...
static __inline__ void display_sched_queue(marcel_lwp_t * lwp)
{
	...
	if (t) {
		...
		do {
			...
			mdebug_sched_q
			    ("\t\t\t\tThread %p (num %ld, LWP %d, ext_state %.8x, sf %.8lx)\n",
			    t, t->number, LWP_NUMBER(GET_LWP(t)),
			    t->sched.internal.entity.ext_state, t->flags);
#else
			mdebug_sched_q
			    ("\t\t\t\tThread %p (num %ld, LWP %d, fl %.8lx)\n",
