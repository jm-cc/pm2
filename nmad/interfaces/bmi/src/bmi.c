 ...
 */
#ifdef __STATIC_METHOD_BMI_NMAD__
extern struct bmi_method_ops bmi_nmad_ops;
#endif
		   ...
		   int flags)
{
	...
	int numreq = BMI_split_string_list(&requested_methods, method_list);
	if (numreq < 1)
	{
	...
	/* Today is that day! */
	addr_count = BMI_split_string_list(&listen_addrs, listen_addr);
	...
	for (i=0; i<numreq; i++) {
		...
		TBX_ERRORF("%s: Failed to find an appropriate listening "
	...
	if(listen_addrs)
	{
	    BMI_free_string_list(listen_addrs, addr_count);
	    listen_addrs = NULL;
	}
    }
    marcel_mutex_unlock(&active_method_count_mutex);
                  ...
                  bmi_hint hints)
{
    ...
    *id = 0;
    ...
    ret = tmp_ref->interface->post_send(
        ...
        user_ptr, context_id, (void*)hints);
    return (ret);
}
