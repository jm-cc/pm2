...
static       int param_min_pack_number   =  1;
static       int param_max_pack_number   =  1;
static const int param_pack_size         =  64*1024; 
static const int param_min_msg_number    =  1;
static const int param_max_msg_number    =  1;
     ...
     ntbx_process_lrank_t lrank_dst)
{
  ...
  for (size = param_min_size;
	   ...
	   msg *= 2)
	{
	  ...
	  if(param_pack_size != 0)
	    {
	      if(size <=  param_pack_size)
		{
		  param_min_pack_number = 1;
		  param_max_pack_number = 1;
		}
	      else
		{
		  if(size%param_pack_size == 0)
		    {
		      param_min_pack_number = size / param_pack_size;
		      param_max_pack_number = size / param_pack_size;
		    }
		  else
		    {
		      param_min_pack_number = (size / param_pack_size) + 1;
		      param_max_pack_number = (size / param_pack_size) + 1;
		      _last_size = size%param_pack_size;
		      _last_size += tbx_aligned(_last_size, MAD_LENGTH_ALIGNMENT);
		    }
		}
	    }
	  ...
	  for (pack  = param_min_pack_number;
	       ...
	       pack *= 2)
	    {
	      const int         _length = tbx_aligned((!size && param_no_zero)?1:size,MAD_LENGTH_ALIGNMENT);
	            int         _size   = tbx_aligned(_length / pack / msg, MAD_LENGTH_ALIGNMENT);
	      ...
	      if (param_one_way)
		{
		  ...
		  while (nb_tests--)
		      ...
		      for (j = 0; j < msg; j++)
			{
			  ...
			  for (i = 0; i < pack; i++)
			    {
			      if(i == (pack - 1) && (param_pack_size != 0)&&(_last_size != 0))
				{
				  mad_pack(out, ptr, _last_size,
