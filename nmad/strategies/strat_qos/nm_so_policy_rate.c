...
struct nm_so_policy_rate_gate{
  ...
  struct list_head pack_ctrl_out_list;  
  unsigned nb_packets;
  unsigned round;
};
...
static int switch_list(struct nm_so_policy_rate_gate *p_so_lg, struct list_head **p_out_list)
{
...
init_gate(void **addr_private)
{
  ...
  INIT_LIST_HEAD(&priv->out_list[NM_SO_POLICY_RATE_PACKS]); 
  INIT_LIST_HEAD(&priv->out_list[NM_SO_POLICY_RATE_CTRLS]); 
  priv->nb_packets = 0;
  priv->round = 0;
