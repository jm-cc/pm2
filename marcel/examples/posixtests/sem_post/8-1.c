...
int set_my_prio(int priority)
{
	...
	{
		fprintf(stderr,"Error at sched_setscheduler()\n");
		return -1;
	}
	return 0;
}
...
int get_my_prio()
{
	...
	{
		fprintf(stderr,"Error at sched_getparam()\n");
		return -1;
	}
...
{
	sem_t _sem, *sem=&_sem, _sem_1, *sem_1=&_sem_1;
   ...
   //if (set_my_prio(priority) == -1)
	...
	if( ret != 0 || sem == NULL ) {
		fprintf(stderr,ERROR_PREFIX "sem_init : sem");
		return -1;
	}
	ret = sem_init(sem_1,0,1);
	if( ret != 0 || sem_1 == NULL ) {
	  fprintf(stderr,ERROR_PREFIX "sem_init : sem_1");
	  return -1;
	}
	sem_wait(sem_1);
	...
	if (sem_wait(sem) == -1)
	{
		fprintf(stderr,"Error at sem_wait");
...
int main() 
{
   ...
   int ret;
	...
	ret = sem_init(sem,0,1);
	if( ret != 0 || sem == NULL ) {
		fprintf(stderr,ERROR_PREFIX "sem_init");
		return PTS_UNRESOLVED;
	}
	
	ret = sem_init(sem_1,0,val);
	if( ret != 0 || sem_1 == NULL ) {
		fprintf(stderr,ERROR_PREFIX "sem_init: sem_1");
		return PTS_UNRESOLVED;
	}
	...
	/* Lock Semaphore */
	if( sem_wait(sem) == -1 ) {
		fprintf(stderr,ERROR_PREFIX "sem_wait");
		retval = PTS_UNRESOLVED; 
		goto clean_up;
	}
