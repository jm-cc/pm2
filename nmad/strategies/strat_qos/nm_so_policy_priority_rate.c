...
struct nm_so_policy_priority_rate_gate{
	  ...
	  union nm_so_generic_ctrl_header *p_ctrl)
{
  ...
  struct tbx_fast_list_head *out_list =
    &((struct nm_so_policy_priority_rate_gate *)private)->pack_ctrl_out_list[priority];
  ...
  ((struct nm_so_policy_priority_rate_gate *)private)->nb_packets++;
 out:
  return err;
}
     ...
     void *data, uint32_t len)
{
  uint8_t priority = nm_so_get_priority(tag);
  struct tbx_fast_list_head *out_list =
    &((struct nm_so_policy_priority_rate_gate *)private)->pack_out_list[priority];
...
try_and_commit(struct nm_gate *p_gate, void *private)
{
  ...
  for(i = 0; i < NM_SO_NB_PRIORITIES; i++)
    {
      struct tbx_fast_list_head *out_list =
	&((struct nm_so_policy_priority_rate_gate *)private)->pack_ctrl_out_list[i];
      struct nm_pkt_wrap *p_so_pw;
     
      if(tbx_fast_list_empty(out_list))
	{
	  out_list = &((struct nm_so_policy_priority_rate_gate *)private)->pack_out_list[i];
...
{
  struct nm_so_policy_priority_rate_gate *priv = 
    TBX_MALLOC(sizeof(struct nm_so_policy_priority_rate_gate));
  uint8_t i;
...
{
  struct nm_so_policy_priority_rate_gate *priv = addr_private[0];
...
busy(void *private)
{
  return ((struct nm_so_policy_priority_rate_gate *)private)->nb_packets;
}

nm_so_policy nm_so_policy_priority_rate = {
  .pack_ctrl = pack_ctrl,
  .pack = pack,
