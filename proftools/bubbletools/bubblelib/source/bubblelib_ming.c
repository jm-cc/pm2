...
static BubbleMovie mynewSWFMovie(void) {
	...
	{ /* stop / continue action */
		...
		SWFShape_drawLineTo(shape,0,0);
		SWFButton_addCharacter(button, shape, SWFBUTTON_HIT);
		...
		button = newSWFButton();
		SWFButton_addCharacter(button, newArrow(BSIZE,BSIZE,1), SWFBUTTON_UP|SWFBUTTON_DOWN|SWFBUTTON_OVER|SWFBUTTON_HIT);
		SWFButton_addAction(button, 
	newSWFAction(" nextFrame(); if (!stopped) play();")
		...
		SWFButton_addCharacter(button, newArrow(BSIZE,BSIZE,1), SWFBUTTON_UP|SWFBUTTON_DOWN|SWFBUTTON_OVER|SWFBUTTON_HIT);
		SWFButton_addAction(button, 
	newSWFAction(" for (i=0;i<16;i++) nextFrame(); if (!stopped) play(); ")
				, SWFBUTTON_MOUSEUP);
		item = SWFMovie_add(movie->swf,(SWFBlock)button);
		SWFDisplayItem_moveTo(item,MOVIEX-2*BSIZE,MOVIEY-BSIZE);

		button = newSWFButton();
		SWFButton_addCharacter(button, newArrow(BSIZE,BSIZE,0), SWFBUTTON_UP|SWFBUTTON_DOWN|SWFBUTTON_OVER|SWFBUTTON_HIT);
		SWFButton_addAction(button, 
	newSWFAction(" for (i=0;i<16;i++) prevFrame();  if (!stopped) play();")
				, SWFBUTTON_MOUSEUP);
		item = SWFMovie_add(movie->swf,(SWFBlock)button);
		...
		SWFButton_addCharacter(button, newArrow(BSIZE,BSIZE,0), SWFBUTTON_UP|SWFBUTTON_DOWN|SWFBUTTON_OVER|SWFBUTTON_HIT);
		SWFButton_addAction(button, 
	newSWFAction(" prevFrame();  if (!stopped) play();")
				, SWFBUTTON_MOUSEDOWN|SWFBUTTON_OVERDOWNTOIDLE);
		item = SWFMovie_add(movie->swf,(SWFBlock)button);
...
static void mySWFMovie_pause(BubbleMovie movie, coordinate_t sec) {
	...
	if (!sec) {
		...
		SWFButton_addCharacter(button, shape, SWFBUTTON_UP|SWFBUTTON_DOWN|SWFBUTTON_OVER|SWFBUTTON_HIT);
		SWFButton_addAction(button, newSWFAction("stopped=1; stop();"), 0xff);
		item = SWFMovie_add(movie->swf, (SWFBlock)button);
...
static void init(void) {
	...
	font = newSWFFont_fromFile(fontpath);
	if (!font) {
	...
	/* pause macro */
	stop = newSWFAction(" if (!stopped) { stop(); stopped=1; } else { play(); stopped=0; }");
}
