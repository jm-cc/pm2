...
void large_contigu_vers_large_contigu(void){
  ...
  *dst = '\0';
  ...
  if (is_server) {
    ...
    /* server */
    nm_so_sr_irecv(sr_if, NM_SO_ANY_SRC, 1, recv_plop, strlen(plop)+1, &request0);

    int i = 0;
    while(i++ < 100000)
      nm_so_sr_rtest(sr_if, request0);

    nm_so_sr_irecv_iov(sr_if, NM_SO_ANY_SRC, 0, iov, 1, &request1);
...
void large_contigu_vers_large_disperse(void){
  ...
  if (is_server) {
    ...
    nm_so_sr_irecv_iov(sr_if, NM_SO_ANY_SRC, 0, iov, 3, &request1);
    nm_so_sr_rwait(sr_if, request1);
...
void large_disperse_vers_large_disperse_disymetrique(void){
  ...
  if (is_server) {
    ...
    nm_so_sr_irecv_iov(sr_if, NM_SO_ANY_SRC, 0, iov, 2, &request1);
    nm_so_sr_rwait(sr_if, request1);
