...
static int explode_sched_init(marcel_bubble_sched_t * self TBX_UNUSED)
{
	return 0;
}

static int explode_sched_exit(marcel_bubble_sched_t * self TBX_UNUSED)
{
	return 0;
}

static int
explode_sched_submit(marcel_bubble_sched_t * self TBX_UNUSED, marcel_entity_t * e)
{
...
static int
explode_sched_vp_is_idle(marcel_bubble_sched_t * self TBX_UNUSED, unsigned vp TBX_UNUSED)
{
	/* Forcer une remontée ? */
	return 0;
}

static marcel_entity_t *explode_sched_sched(marcel_bubble_sched_t * self TBX_UNUSED,
					    marcel_entity_t * nextent, ma_runqueue_t * rq,
					    ma_holder_t ** nexth TBX_UNUSED, int idx)
{
	int max_prio;
	ma_runqueue_t *currq;
	marcel_entity_t *e;
	...
	if ( /*ma_idle_scheduler && */ nextent->sched_level > rq->level) {
		...
		for (; currq; currq = currq->father) {
			...
			if (idx < max_prio) {
				MARCEL_SCHED_LOG
				    ("better prio %d on rq %s in the meanwhile\n", idx,
				     currq->as_holder.name);
				MARCEL_SCHED_LOG("unlock(%p)\n", rq);
				ma_holder_unlock(&rq->as_holder);
				return NULL;
		...
		for (currq = ma_lwp_vprq(MA_LWP_SELF); currq &&
		     nextent->sched_level < currq->level; currq = currq->father);

		MA_BUG_ON(!currq);
		PROF_EVENT2(bubble_sched_switchrq,
			    nextent->type == MA_BUBBLE_ENTITY ?
			    (void *) ma_bubble_entity(nextent) :
			    (void *) ma_task_entity(nextent), currq);
		MARCEL_SCHED_LOG("entity %p going down from %s(%p) to %s(%p)\n", nextent,
				 rq->as_holder.name, rq, currq->as_holder.name, currq);
...
#endif
	ma_atomic_set(&bubble->as_entity.time_slice,
		      MARCEL_BUBBLE_TIMESLICE * _TIME_SLICE);
#undef _TIME_SLICE
	MARCEL_SCHED_LOG("timeslice %u\n", ma_atomic_read(&bubble->as_entity.time_slice));

	tbx_fast_list_for_each_entry(e, &bubble->natural_entities, natural_entities_item) {
		/** Check if the entity is a bubble and lock it before moving it ! */
...
static int
explode_sched_tick(marcel_bubble_sched_t * self TBX_UNUSED,
