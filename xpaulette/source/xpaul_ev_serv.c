...
/* Renvoie MARCEL_SELF si on a le lock
   NULL sinon
*/
inline static marcel_task_t* xpaul_ensure_lock_server(xpaul_server_t server)
{
...
inline static void xpaul_restore_lock_server_unlocked(xpaul_server_t server,
					        marcel_task_t *old_owner)
{
	if (old_owner) {
 ...
 */
inline static int __xpaul_poll_group(xpaul_server_t server, xpaul_req_t req)
{
#ifdef MA__DEBUG
	...
	if (server->funcs[XPAUL_FUNCTYPE_POLL_GROUP].func &&
		...
		xdebug("Factorizing %i polling(s) with POLL_GROUP for [%s]\n",
...
inline static void __xpaul_poll_start(xpaul_server_t server)
{
	...
	if (server->poll_points & XPAUL_POLL_AT_TIMER_SIG) {
		xdebug("Starting timer polling for [%s] at frequency %i\n",
...
inline static int __xpaul_wake_id_waiters(xpaul_server_t server, int code)
{
	...
	list_for_each_entry_safe(wait, tmp, &server->list_id_waiters, chain_wait) {
		...
		case 0:
			...
			break;
		default:
			xdebug("Poll awake with global task %p on code %i\n", wait->task, code);
		}
#endif
		wait->ret=code;
		list_del_init(&wait->chain_wait);
...
inline static int __xpaul_unregister(xpaul_server_t server, xpaul_req_t req)
{
	LOG_IN();
...
static void xpaul_check_polling_for(xpaul_server_t server)
{
	...
	if(nb == 1 && server->funcs[XPAUL_FUNCTYPE_POLL_POLLONE].func) {
		...
		(*server->funcs[XPAUL_FUNCTYPE_POLL_POLLANY].func)
			(server, XPAUL_FUNCTYPE_POLL_POLLANY,
			 NULL, nb, 0);
...
int xpaul_req_attr_set(xpaul_req_t req, int attr)
{
	...
	if (attr & (~(XPAUL_ATTR_ONE_SHOT|XPAUL_ATTR_NO_WAKE_SERVER))) {
...
int __xpaul_need_export(xpaul_server_t server, xpaul_req_t req, xpaul_wait_t wait,
		     marcel_time_t timeout)
{
  /*********************************************
		   ...
		   xpaul_wait_t wait, marcel_time_t timeout)
{
	...
	if (__xpaul_need_export(server, req, wait, timeout)) {
	  ...
	  (*server->funcs[XPAUL_FUNCTYPE_BLOCK_WAITONE].func)(server, XPAUL_FUNCTYPE_POLL_POLLONE,
						      req, server->req_poll_grouped_nb, 1);
#ifdef MARCEL
