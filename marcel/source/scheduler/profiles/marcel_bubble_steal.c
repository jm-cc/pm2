...
static marcel_bubble_t *find_interesting_bubble(ma_runqueue_t * rq, int up_power)
{
	...
	for (i = 0; i < MA_MAX_PRIO; i++) {
		tbx_fast_list_for_each_entry_reverse(e, ma_array_queue(rq->active, i),
			...
			if ((nbrun = total_nb_ready_entities(b)) >= up_power) {
				MARCEL_SCHED_LOG
				    ("bubble %p has %d running, better for rq father %s with power %d\n",
...
static int see(struct marcel_topo_level *level, int up_power)
{
	...
	if (!b) {
		...
		MARCEL_SCHED_LOG("no interesting bubble\n");
		return 0;
	}
	...
	for (rq2 = rq;
	     ...
	     && marcel_vpset_weight(&rq2->father->vpset) <= nbrun; rq2 = rq2->father)
		MARCEL_SCHED_LOG("looking up to rq %s\n", rq2->father->as_holder.name);
	if (!rq2 ||
	    /* pas intéressant pour moi, on laisse les autres se débrouiller */
	    /* todo: le faire quand même ? */
	    !marcel_vpset_isset(&rq2->vpset, ma_vpnum(MA_LWP_SELF))) {
		MARCEL_SCHED_LOG("%s doesn't suit for me and %d threads\n",
				 rq2 ? rq2->as_holder.name : "anything", nbrun);
		ma_holder_rawunlock(&rq->as_holder);
		return 0;
	}

	MARCEL_SCHED_LOG("rq %s seems good, stealing bubble %p\n", rq2->as_holder.name,
			 b);
	ret = 1;
	...
	tbx_fast_list_for_each_entry(e, &b->natural_entities, natural_entities_item) {
		if (e->type == MA_BUBBLE_ENTITY) {
			...
			e->settled = 1;
			MARCEL_SCHED_LOG("detaching running bubble %p from %p\n", b2, b);
		} else {
			t = ma_task_entity(e);
			/* ne pas toucher à ce qui tourne actuellement */
			if (!MA_TASK_IS_RUNNING(t))
				continue;
			MARCEL_SCHED_LOG("detaching running task %p from %p\n", t, b);
		}
	...
	tbx_fast_list_for_each_entry(e, &b->natural_entities, natural_entities_item) {
		...
		if (e->type == MA_BUBBLE_ENTITY)
			ma_holder_rawunlock(&b2->as_holder);
...
static int see_down(struct marcel_topo_level *level, struct marcel_topo_level *me)
{
	ma_runqueue_t *rq = &level->rq;
	int power = 0;
	int i = 0, n = level->arity;
	MARCEL_SCHED_LOG("see_down from %d %d\n", level->type, level->number);
	if (rq) {
...
static int see_up(struct marcel_topo_level *level)
{
	...
	MARCEL_SCHED_LOG("see_up from %d %d\n", level->type, level->number);
	if (!father)
		return 0;
 ...
 */
static int steal_work(marcel_bubble_sched_t * self TBX_UNUSED, unsigned vp TBX_UNUSED)
{
#ifdef MA__LWPS
	ma_spin_lock(&ma_idle_scheduler_lock);
	if (ma_atomic_read(&ma_idle_scheduler)) {
		struct marcel_topo_level *me = marcel_current_vp_level();
		MARCEL_SCHED_LOG("bubble steal on %d\n", ma_vpnum(MA_LWP_SELF));
...
static marcel_entity_t *steal_sched_sched(marcel_bubble_sched_t * self TBX_UNUSED,
					  marcel_entity_t * nextent, ma_runqueue_t * rq,
					  ma_holder_t ** nexth TBX_UNUSED,
					  int idx TBX_UNUSED)
{
	...
	if (ma_idle_scheduler_is_running() && !nextent->settled) {
		...
		nextent->settled = 1;
		if (rq != rq2) {
			MARCEL_SCHED_LOG("settling bubble %p on rq %s\n", bubble,
					 rq2->as_holder.name);
...
static int steal_sched_start(marcel_bubble_sched_t * self TBX_UNUSED)
{
